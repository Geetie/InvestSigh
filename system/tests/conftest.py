...
# ★ `raw` 是**批次 5（卡 13-F / `G-54`）补的**，理由是**实测代价**，不是推测：
#   夹具副本里的 `raw/` **必须是空的** —— `_reset_truth_source()` 正是把它清空的那一步。
#   但"复制进来再逐项删掉"要按**删除调用次数**付费：宿主对每次 `unlink()`/`rmtree()`
#   收 ≈0.5~0.8s **固定**审批往返（与删几项无关）。本目录有 5 项残留 ⇒ **≈4.2s/例**
#   （实测 `_reset_truth_source` 的 `raw` 段 = 4.157s），而它删完的结果与"根本不复制"**完全一样**
#   （都是空目录；`_ENSURE_DIRS` 会把 `raw/` 补回来）。
#   ⇒ 与 `index` / `reports` / `derived` **同等待遇：不复制**。
#   ★ 与它们不同的是：那三个是"可重建产物"，而 `raw` 是**真源**（外部文本只进 `raw/`，
#     见 `Ch9 §3.3.3`）—— 不复制它的理由**不是**"可重建"，而是
#     「**夹具契约要求它为空**，复制一份再去删是纯浪费」。这两条理由必须分清，
#     否则下一个人会把 `raw` 当成无关紧要的缓存目录。
#   ★ 行为等价性已实测：改动前后夹具里 `raw/` 都只含**空目录**（`raw/inbox` 的 `.gitkeep`
#     本来就被 `_ignore` 的点号规则跳过）⇒ `injection_guard._raw_has_external_text()` 两边都是 `False`。
_COPY_SKIP = {
...
def _reset_truth_source(target: Path) -> None:
    ...
    ★★ **本函数的 `raw/` 清理是"每例 ~4.2s"的来源，且成本与"项数"无关**（`G-54` 实测）：
      宿主对**每一次删除调用**（`Path.unlink()` / `shutil.rmtree()`）收一笔**固定 ≈0.5~0.8s**
      的审批往返，**与本次删几项无关**。实测（broker 关，本工作树）：

      | 操作 | 耗时 |
      |---|---|
      | `k` 次独立 `unlink()`（k=1/2/4/8） | 0.83 / 1.20 / 2.02 / **4.33** s ⇒ **≈0.54s × 调用次数** |
      | **1** 次 `rmtree()`（n=50/150/310 项） | 0.86 / 0.74 / 0.76 s ⇒ **与项数无关**（n=310 时边际 2.4ms/项） |
      | 写 310 个文件（对照） | **0.038s**（写不进 broker） |

      原实现对本目录的 5 个残留项**逐项**删（4 次 `unlink` + 1 次 `rmtree`）⇒ **≈4.2s/例**，
      而删掉整个 310 项的夹具目录只要 ~0.76s（**一次**调用）。
      ⇒ 处置：**让 `raw/` 的内容根本不进副本**（`raw` 已加入 `_COPY_SKIP`）——
      副本里 `raw/` 本来就是空的（本函数会把它清干净），**复制它纯属白付**。
      本函数保留逐项清理作为**兜底**（例如有人把 `raw` 从 `_COPY_SKIP` 里删掉）。

      ★ **不要**在这里把循环改成"更少次数的删除"以外的优化，也不要指望"树变小了就快"：
      真正的自变量是**删除调用次数**，不是文件数。
    """
    facts = target / "facts"
    if facts.is_dir():
