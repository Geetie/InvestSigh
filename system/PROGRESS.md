...
**★ 第五轮已裁定：采纳 (甲)**，落地与证据见下方 **§12.4**（本条闭合）。
...
### 12.4 ★ 第五轮：主理人**裁定（甲）**落地 —— `value_source` 的**声明粒度**收紧

`§④-8a`（第三轮起提了三轮）主理人裁定 **采纳 (甲)**：`value_source` 是对**整个结构**的"已核"声明
⇒ **"全部字段都来自文件"才叫 `rules`**，否则 `design_default`（缺件事实由 `notes` 保留，**不丢信息**）。
理由（他的原话）= *"标志的单位必须与它声称覆盖的单位一致"*，落在 **`V-11` 类别轴** / **`G-62` 不可区分**上。
...
**★ 反向对照（`G-05`，主理人明确要求）**：字段齐备时**必须仍报 `rules`** —— 否则标志退化成
"永远 `design_default`"，与"永远 `rules`"**一样没有信息量**。实测三处**全部仍报 `rules` 且零 note**。

**★★ 落 (甲) 时"必须同步动"的三处**（漏一处就变成"改出新的静默"）：

| # | 连带改动 | 为什么非改不可 |
|---|---|---|
| ① | `solver`：`int(node.get(k) or DEFAULT)` → **先判键在不在**；键在但值非整数 ⇒ `PriceLayerError` | 旧写法在**键在但值为 `0`/`None`** 时用 `or` 悄悄换成回落值**且不留 note** ⇒ 标志会**误报 `rules`**（正是甲要堵的洞） |
| ② | `solver`：`must_show_multiple` 由 `bool(node.get(k, DEFAULT))` → **键在且非布尔 ⇒ `PriceLayerError`** | `bool(None)` = `False` ⇒ `must_show_multiple: null` 会把"必须多组解"这条**设计不变量**悄悄关掉（下限退化为 1） |
| ③ | ★★ `daily_explain._rule_binding_violations`：把 `on_hit.…` 的**存在性判据挪到早退之前** | 甲生效后"缺该子键"⇒ `value_source` 变 `design_default`，而该函数有一条"`value_source != rules` ⇒ 报读不到值并**早退**"。存在性判据若排在早退**之后**就会被**吞掉**，只剩笼统消息、**信息更少** —— 与 `valuation.check` 的 **D-1「早退吞违例」同一形状**（`G-62` 家族） |

★ ③ = 主理人点名赞扬过的那条**跨模块形状**（"早退吞违例"）的**第二次复现**（修 8 那次是第一次），
且是**在同一个人、同一个补丁里**又踩到一次、又自己发现并挪走。
★ 挪走后原位置留下**同一段判据的第二份副本**（我第一轮编辑时留下的重复块）——按**修 5/修 6 的同一纪律**
（死代码 / 重复代码必须删）**已删**。

**★ 口径变更史（写明"为何不是反复"）**：第三轮裁定 ③-2 定的是**实现口径**（"节存在即 `rules`"，即**怎么判**）；
第五轮 (甲) 定的是**声明的粒度**（"标志只能覆盖它声称覆盖的单位"，即**声称什么**）⇒ **同向收紧，非翻转**。

**★ 本流的一个选择 —— 主理人裁定：**保留纳入**，但加一条**可区分性条件**（`G-62`）**：
`solution_set_display` 的 `selection`/`overflow` 是**自由文本标签**、**无设计逐字回落值**，严格看不构成"回落"；
但 (甲) 的字面是"**全部字段都来自文件**才叫 `rules`"，且 `""` 确实**不是从文件读到的** ⇒ 本流**选择纳入**
（**宁严勿松**：只会多报、不会漏报）。
**★ 附加条件**：两种情况**在读数上必须可区分**，否则下游会误以为"有一个**被设计认可过的**默认值被应用了"。
落地 = **token 分设**（不只改措辞）：

| 情形 | token | note 必须能读出 |
|---|---|---|
| **有**设计逐字依据的字段 | `NOTE_NO_DISPLAY_KEY` | **用了哪个**设计逐字回落值 + "有设计逐字依据" |
| 设计里**本无**逐字值的标签（`selection`/`overflow`） | `NOTE_NO_DISPLAY_LABEL` | **本无**回落值、只是空串 |

⇒ 由 `test_display_fallback_note_distinguishes_the_two_kinds` **机器绑定**（token 改名 / 混用 ⇒ 当场红），
含**反向对照**：字段齐备时两种 token **一个都不出现**。

**★ 证据（报告 §② V-1l）**：8 例反例（删任一子键，旧 `rules` → 新 `design_default` **且点名该键**）+
3 例反向对照（字段齐备 ⇒ 旧新**都** `rules`）+ 真文件上四门禁 CLI 全 `exit=0`（计数 = 1）+
"早退吞违例"的具名 / 笼统**两条 FATAL 都在**（挪走后具名那条才不会被吞）+
**可区分性原样输出**（`NO_DISPLAY_KEY` 带具体回落值 3/5/True；`NO_DISPLAY_LABEL` 写明"本无"）。
★ 取证方式同前：`/tmp` 里真 `rules/` 的**只读副本**做变异根（原文件 0444，**从未被写过**）；
**旧 = `git show HEAD:…` 的字节** `exec` 成 `scripts.pricelayer._old_*`（工具名 = `git`，非复述）。

**★ 全量复核（同 V-1k 手法，零删除口径，不经 pytest 命令）**：`tests/pricelayer/**` 8 模块 + parametrize 逐组展开
⇒ **202 通过 / 失败 0**（第四轮 196 → 裁定甲 +5 → 可区分性机器绑定 +1；`test_solver.py` 29→35，
其余 7 个模块逐模块**不变**）。合并 `main` 后重跑一次**逐模块相同**。★ 仍**不是**"批次通过"
（无收集 / 无 fixture 终结器）；正式读数待窗口关闭后由 `#83` 轮 1 统一取（主理人指定，**不自起会话**）。

### 12.5 ★ 引用复核（并入 `main` 后）：本报告自己的绝对行号引用 **6 处失效**，**两种机理分开**

**触发**：工作树并入 `main`（`da45c2e → 1245080`，fast-forward；按 `V-07` 重跑 `bootstrap_worktree.sh`，
14 个 `rules` 复 0444、`rules_lock_guard` PASS、`system/rules|registry` 的 `status`/`diff` 均零输出）后，
按 `V-11` 规则 9 把报告里全部 `<文件>:<行号>` 引用**逐条重核**（不抽样）。

**结果**（仪器 `/usr/bin/grep -nE` / `python3` / `git log -S` / `git show` + `awk`，逐条 rc=0；详见报告 §② `V-1n`）：
- **4 处"漂"**（写下**当时成立**，被其**上方**的增删行挤走）：`valuation.py:201/577 → 211/600`、
  `models.py:1706 → 1753`、`models.py:1077-1081 → 1075-1082`；
- **2 处"引错"**（**在写下它的那次提交上就不成立**）：`verify.py:620-625`（引到了 docstring 的 broker 段，
  载荷实为 `649`）、`scenario_guard.py:195`（引到了 docstring，首轮载荷实为 `91`）。
- 2 处**未漂**原样保留：`run_pytest.sh:35`、`batch13_taskbook.md:412`。

★★ **两类不可混报**：混成"行号都过期了"会把 **2 处真写错**洗成"正常时效问题"（`G-62` 又一落点）。
★ 对策（只改写法）：载荷**按符号/内容锚定**，行号降级为"当时读数"且须与对象 SHA 同现。
★ 是否把这条写进 `CONVENTIONS.md::V-11` 规则 9 —— **交主理人裁定**（本流未改该文件）。
