...
## 十三、卡 13-N · `verify.py` 散文里的「分片事实」单源化
...
> **收尾尖端 `4581102`**（见 13.7：首轮漏了 2 处 + 补守卫第 ④ 条判据）；其后只有本报告/本节的 **docs-only** 提交。
...
（"同一事实两处写必然漂移"）的**第 4 次复发**，且**本处是主理人自己造成的**。

**处置口径（本卡新立，决定了每一处取舍）**：**"描述当前事实"的数字必然漂移 ⇒ 删；"记录某次实测"的数字不会漂移 ⇒ 留。**
能删就删；删不掉的从真源派生（并给**查真值命令**）；**不许在散文里再抄一份计数** —— 包括我自己新写的声明段里那句"**8 次**"（引用了主理人的计数 ⇒ 又抄一份，随即自删）。

### 13.1 已落地

| 项 | 内容 |
|---|---|
| **散文清理 21 处** | 片数 11 处（`:43 :72 :91 :94 :226 :228 :237 :238 :313 :516 :518`）· 枚举上界 1 处（`:29` 的 `injection-a…injection-f`）· 旧分片数 2 处（`:362 :383`）· **顺带的同类漂移 8 处**（`165 例` / `27 → 30` / `injection-b(28),injection-d(29)` / `取上限 300s` …）· **1 处活的自相矛盾**（`unit` 批散文写 `270s`、参数是 `120.0`） |
| **新增声明段 2 处** | ① docstring 表下与既有"超时值不在本表重复"并列的"**片名单/枚举上界/片数同样不在本表重复**"；② `BATCHES` 前的"**读本字典前必读**"：超时值的唯一真源 = `Batch(...)` 的位置参数，注释里的秒数一律是**取值沿革/留档** |
| **新增守卫** | `tests/guards/test_verify_prose_binding.py`（10 用例，**零 `code_root` 夹具**）；三条判据**全部由 `INJECTION_SHARDS` 现算**（片名集合、含"片字母"由后缀现算），覆盖 ①分片名单 ②片数/轮次写法 ③旧分片数写法；配 **4 条活体反向对照**（逐条写回 ⇒ 立即红，自动还原并校验 `sha256`）与 1 条"反向的反向"（惯用语「一片」/ 单个片名交叉引用 / 十六进制片段不得误报） |
| **零代码改动（机器可核）** | 剥掉「注释 + 模块 docstring」后逐 token 比对：**1879 / 1879 相同**；AST（剥 docstring）相同 ⇒ 功能位与**每一个超时值**逐字未动 |
| **未触碰** | `tests/injection/**` 一个字都没动（**b/e 两片余量 0** 的硬约束）；新守卫落 `tests/guards/`（目录式目标，自动被 `guards` 批覆盖 ⇒ `test_files_uncovered: 0`，且**不消耗注入分片配额**） |
...
### 13.3 ★ `main` 上 `injection-f` 的红（我开工时报双红，现只剩一条）

`python -m pytest tests/injection/test_shard_coverage.py -q`：开工基线 `ed9687b` ⇒ **2 failed, 4 passed**；
交付基线 `56fd637` ⇒ **1 failed, 5 passed**。

1. ~~`test_missing_pytest_attribution_never_passes`~~ —— **已被 `main` 修掉**（我报出后 main 前进的
   22 个提交里补回了实现）。原根因：`ea0ceae`（"merge ws/verify-shard（口径 11 补合）—— **冲突取主干侧**"）
   对 `verify.py` 取主干侧，丢掉了 `90e58b0` 新增的 `PYTEST_MISSING_MARKER` + `_exit_zero`
   环境归因分支；而同一合并对 `test_shard_coverage.py` **取分支侧**，把断言留下
   ⇒「**测试有了、实现没了**」。
2. **仍红**：`test_shard_case_counts_within_quota` ⇒ `{'injection-f': 33}（现算值）`
   —— 同一个补合并把该文件从 4 例带到 6 例，而它**自己就住在 f 里** ⇒ f: 31 → 33 > 上限。
   **未擅动**（`test_shard_coverage.py:110` 逐字写着"片数是主理人决策"），已上报请裁。

★ **教训（建议升格为口径）**：**冲突解决的正确粒度不是"文件"，而是"被同一个改动同时触及的事实集合"。**
"逐文件各取一侧"看起来是最小冲突面，实际会在跨文件耦合处留下**一半实现 + 一半断言**。

**现算分片读数（`ed9687b`，`--collect-only --noconftest`，脚本 `/tmp/n13/measure.py`）**：
`a=30 b=32 c=31 d=29 e=32 f=33 g=18`，逐片求和 = 整目录 = **205**；**`f` 余量 −1**。
⇒ 主理人写进 13-J 的硬约束「新用例落 b/e 必红」应扩为「**落 a–g 任一片都可能红，其中 f 已红**」。

### 13.4 剩余缺口（逐条见报告 §四）

① 任务书说"8 处"、我实测 **11 处**（按行逐命中，以脚本输出为凭）；② **超时值复述未逐个删除**
（只做了"修矛盾 + 统一降格为留档并声明真源"，因倍数离开数值不成立）；③ 守卫是正则，
**非穷尽于自然语言表达空间**（诚实登记，建议开 `T` 号）；④ `test_shard_coverage.py` 里有**同族漂移**
（`165 个用例` / `171 例` / `a 27 例 b 28 例 d 31 例 f 31 例` 均已过期），**不在本卡对象内、且该文件当前红着，未擅动**，建议单独开卡。

### 13.5 ★★ 合并时撞上的一件事（**本卡最重要的观察**）：`main` 已经"把数字改对"过一次

交付期间的 22 个提交里，`main` **恰好改过本卡的题眼那一行** —— 把
「`injection-a`…`injection-f` + **6 个分片**」改成「`injection-a`…`injection-g` + **7 个分片**」。
于是本次合并 `verify.py` 只产生 **1 处冲突**，正是这一行。

**我取了自己那一侧（删掉枚举与片数、改指 `INJECTION_SHARDS`）**，理由是本卡守卫红字里那句：
**"不要把数字改对 —— 改对了仍然会再漂移一次"**。下一次开 `injection-h`、或把某片再拆一片，
"7" 会原样过期，而且因为"上次是对的字面"，review 时更难被注意到。
⇒ **「把数字改对」才是这类漂移真正的危险形态**：它把漂移计时器重设了一次，还顺手消除了被发现的线索。
⇒ 这也是 `G-28` 四次复发里**第一次有机器绑定**的原因（此前四次都只靠"下次注意"）。
...
### 13.7 ★★ 收尾自查：**首轮我漏了 2 处**，且漏的正是本卡的题面本身

按 DoD ① 用**当前尖端**重跑清点时，`②枚举上界` 类里冒出两行我没处置过的写法（在 `injection-g` 批注释里，
**由第二次合并 `96d4d7e` 带进来**；我在 `5fe0879` 那轮复核同一段时只改了那个 `90s`，**没看见同一段的枚举上界**）：

| 改前逐字原文（`052cb5f:verify.py`） | 为什么 ① 抓不到 |
|---|---|
| `与 `injection-a..f` **齐平**（同族一致性优先）` | **范围写法**：该行只含 **1 个**全名（`injection-a`），右端是**裸字母** `f` ⇒ ① 的"≥2 个全名"阈值放过它 |
| `而同期 `a..f`（300s）不会` | 同上，**连前缀都没有** |

处置：删枚举（**不**改成 `a..g` —— 改成对的上界同样是第二存放处，见 13.5），
并把同段的 `247s ≫ 90s` 拆为 `247s（远超旧值 90s）`，让 `90s` 只以**沿革**姿态出现（13.6 的中间态口径）。

**为此新守卫补第 ④ 条判据**（从真源派生，不写死字母）：
> 散文里的**范围式枚举**（`injection-a..f` / `a..f`）**右端 ≠ 最大后缀** ⇒ 违规；
> **右端 = 最大后缀（`a..g`）刻意放行** —— 不与真源冲突，若连它也判红，正当注释会被逼到无法书写（`G-01`）。

★ 补 ④ 时，**我自己的"反向的反向"用例当场抓到 ④ 的一个 bug**：`re.split` 拆范围时，
`injection-a..g` 里的**连字符**把左端拆成了 `injection` ⇒ 正确写法被误报。改为**正则捕获组**取两端，
并把这条 bug 写进 docstring。⇒ **"必须写反向的反向用例"的实证**：没有它，守卫会以"过宽"的形态上线。

**实测**（尖端 `4581102`）：`11 passed`（首轮 10）· 活体反向对照 **6/6 成立**（新增 ④ 两条）·
改前/改后清点 **53 → 52**（② 类 29 → 28）· 剥离全部注释与 docstring 后与 `main` 的**代码 token 流逐一相同（1867/1867）**。

★★ **对全队的教训（比本卡的数字更重要）**：
1. **"判据覆盖了本文件已出现的全部写法"这句话本身也会过期** —— 首轮它就是错的；
2. 主理人数"**8 处**"用的数法（按"6 片"字样数）**在结构上就抓不到 `a..f` 这种写法** ⇒
   用"数量"当验收判据，会连"漏在哪一类"都看不见（`口径 19`：判据必须是可观测结果，而不是做法/计数）。

---

## 十四、卡 `#89`（主理人裁定）· `test_shard_coverage.py` 由 `injection-f` 迁入 `injection-g`

> 尖端 **`7c38466`**（其后 docs-only）；与卡 13-N **同机理**（「分片事实的第二存放处」），
> 故证据并入 `reports/ws_ch4_valuelayer_card13n_report.md §六`，不另起文件。

**裁定 (A)**：该文件由 `f` 迁 `g`。**为什么是唯一解**：它 6 例，而各片余量 `a`2 / `b`0 / `c`1 / `d`3 / `e`0 / **`g`14** ⇒ 只有 `g` 装得下。
**为什么不会更糟**：它是**零夹具**文件 ⇒ 迁入的单轮删除成本 ≈ 0（`V-08` 预算不受影响）。
★ 片数/归属是主理人决策（该文件 `:110` 逐字写明）⇒ 我先前**不动**是在等裁定。

**实测**：`test_shard_coverage.py` **6 passed** RC=0 · 逐片现算 `a30 b32 c31 d29 e32 **f27** **g24** = 205 = 整目录`
（**一口不多、一口不少**，纯搬迁）· **超过 32 的片 = `[]`**（真红解除）· `--batch injection-f` **exit=0 / 27 passed** ·
`--batch injection-g` **exit=0 / 24 passed** · `pre-commit` **14 门禁 / 14 PASS / RC=0**。

**同族漂移清理**（主理人两档口径：能派生就派生 / 派生不掉就**显式标注为历史观测值**）：
`:6` `273` → 常量名 `COPIED_ITEMS_PER_CASE` · `:111` 逐片条数与"哪片最接近上限"**全删** ·
`:113`「6 片 + 171 例」→「当前分片」+ **折算需 ≈14 片（写死前提）** · `:242/:313/:332` 三处 `271` → 常量名 ·
`:5` 的 `165` 保留但标注「历史观测值，**不是当前真源**」· 新增段首「**读本段前必读**」（本段数字全是留档）；
**被推翻的推导原文（含 `271`）原样保留**（引用里改数字 = 篡改证据）。

★★ **过程中我的 13-N 守卫抓到了我自己新写的散文**：我写的「迁后**两片**都 ≤ 上限」命中 ② 类（片数写法 `两片`）
⇒ `guards` 批 `2 failed, 98 passed`。改成「迁后**涉及的片都 ≤ 上限**」后 `11 passed`。
**`G-28` 同族的第 5 次复发，第一次由机器在上线前拦住。**

★ **未取得的一项验收（如实登记）**：`guards` 批被**宿主单轮删除配额**挡住 ——
本轮连跑 f(27)+g(24)+guards(98 例) 后，复跑两次都是
`exit=3 … **宿主单轮删除配额耗尽**（scope: "turn"）`。
