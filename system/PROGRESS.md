...
**★ 本流的一个选择（已登记请复核）**：`solution_set_display` 的 `selection`/`overflow` 是**自由文本标签**、
**无设计逐字回落值**，严格看不构成"回落"；但 (甲) 的字面是"**全部字段都来自文件**才叫 `rules`"，
且 `""` 确实**不是从文件读到的** ⇒ 本流**选择纳入**（**宁严勿松**：只会多报、不会漏报）。
若主理人认为应排除，**回退成本 = 该循环少两个键 + 1 条用例**（同循环、同 note 机制）。
...
**★ 第五轮全量复核（同 V-1k 手法，零删除口径，不经 pytest 命令）**：`tests/pricelayer/**` 8 模块 + parametrize 逐组展开
⇒ **201 通过 / 失败 0**（第四轮 196，本轮 +5 = 新增的 5 组 parametrize）。★ 仍**不是**"批次通过"
（无收集 / 无 fixture 终结器）；正式读数待窗口关闭后复跑。
