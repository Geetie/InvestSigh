...
BINDINGS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        # ★ 集成期更正（主理人，批次 13）：原绑 `SCENARIO_METHOD_STATUSES`，而 13-B 的修订轮
        #   **把该常量删掉/改名为 `DESIGN_SCENARIO_METHOD_STATUSES`** —— 它现在被**显式标注**为
        #   「`Ch5 §E.4` 逐字取值域，**仅作**规则文件缺该键时的回落 + note」（见 `scenario_guard.py:125-126`）。
        #   ⇒ 这正是 `G-06` 镜像裁定允许的形态（**副本 + 显式标注 + 有真实回落场景**），
        #     故绑定目标随之改到新符号；**不是**"实现面漂移"。
        #   ★ 两类改动**各自都对**，是本守卫在集成时把"绑定目标已被改名"如实拦下 ——
        #     属**语义层集成冲突**（文本层不冲突），已登记留痕。
