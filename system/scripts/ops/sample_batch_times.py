...
def _print_plan(cells: tuple[Cell, ...], repeat: int) -> None:
    ...
    print("- ★★ 且**计数器不是硬上限**：实测 10/20 个 `rid` **越阈**（最高 **3.85×**），`requestRejections` 为空。"
          "⇒ 「一轮装不下」的**前提是阈值被强制**，而**这一点本单从未证明**（无合法对照：`E=1` 反测已裁不做）"
          "⇒ 下面是「**若强制生效，则…**」的保守预算，**继续按主理人已采纳的分轮打包执行**（不据此改计划）。")
    print("- ★ 按上述两点，**排他的物理含义请以主理人的调度为准**（我自检不了「是否同一 `rid`」）。")
