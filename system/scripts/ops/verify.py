...
## ★ 为什么 `tests/injection/` 必须切成多片（**门禁曾被静默关掉**）
...
★ **代价（如实写明）**：完整覆盖 `tests/injection` 需要**与 `INJECTION_SHARDS` 片数相同的轮次数**
（每轮跑一片 —— 片数不在此处写死，真源就是那个常量）。
这是**宿主单轮删除配额决定的，不是设计选择** —— 同一轮里连跑多片会重新耗尽配额，
表现为"某片在几秒内突然红 + 一堆 `E`"（`scope: "turn"`）。
`--batch all` 会把各片连着跑完，因此它**不能**用来做 `tests/injection` 的完整覆盖。
...
#   `tests/injection` 全目录的用例数 × 每例一份夹具副本 ≫ 宿主单轮配额。
#   ⇒ 完整覆盖该目录需要**与片数相同的轮次数**（每轮一片）。
#   ★ **片数与各片例数一律不在此处写死**：唯一真源 = 下面的 `INJECTION_SHARDS` 常量，
#     例数用 `tests/injection/test_shard_coverage.py` 的**现算**值（`--collect-only`）。
...
#        现登记的片数（每片 ≤32 例）在 **9,999** 这个较低阈值下**不成立**（需 ≈14 片）；
...
BATCHES: Mapping[str, Batch] = {
    "unit": Batch(
        ...
        #
        # ★ 270s 是**故意保留的安全网**（修后实测 15.79s ⇒ 17.1×，比 `V-02` 的 4~8× 宽）：
        #   ① 我**没有**"修后 + 高并发"的实测，不拿未测场景去压它；
        #   ② 主理人明示"不要为了好看把它压回 60s"。
    ...
    #   ★ **最终取值 = `tests/injection/` 全部各片统一 300s**（`V-02` 允许的上限）；理由与上面那条相反方向的取舍：
    ...
    "injection-g": Batch(
        ...
        #   ★ 为什么**不能**塞进既有分片：**当时快照**为 a27 b28 c27 d29 e32 f29（**最大余量只有 5 例**），
        #     而本文件 **17 例** ⇒ 任何既有片都会 > 上限 32 ⇒ 只能**新开一片**。
        #     ★「当时快照」四字是必要的：现算值由 `test_shard_coverage.py` 的断言给出，**不在此处写死**
        #       —— 本卡（#91）修的就是"把当时的例数写成永恒事实"这一类漂移。
        #   ★ 超时 90s：按 13-F 给分片定的规则（实测 × 8 上沿、不低于 90s）；17 例量级与本片
        #     `injection-a`（**当时** 27 例 / 6.85s 隔离）同阶 ⇒ 90s 起步，后已由 13-F 对照表改为 300s。
        ...
        #   会被判 `[TIMEOUT]`，而同期其他各片（300s）不会。⇒ 只是让本片**不比邻片更早红**。
        300.0, _exit_zero,
    ),
    "injection-f": Batch(
    ...
    "compute": Batch(
        ...
        #   ★ 本批**不依赖共享 `code_root` 大夹具**，但 **≠「0 夹具」**（主理人裁定 2026-09-16，卡 `#102`）：
        #     `tests/compute/conftest.py` **自带 2 个 fixture**（`scratch` 建最小工程根、`cli` 跑真子进程），
        #     其中 `scratch` 的 teardown 是 `shutil.rmtree(root.parent, ignore_errors=True)`。
        #     ⇒ ① `D′`（`raw` 入 `_COPY_SKIP`）**与本批无关**（`raw` 在共享大夹具里）；
        #       ② `E` 下两次**独立**读数 **1.49 / 1.42s**（6.3）与 **2.60s**（6.6）—— **同一配置的重复
        #          测量，差值不得归因给守卫**（`口径 12`；两轮都跑在 `E` 下，见 `_child_env`）。
        #          两者都**不为 0**，与"本批自带 `scratch` 的 teardown 真在删除"**一致**
        #          （★ 只报一致性，不做因果归因）。
        #     ⇒ 结论不变：本批的耗时主要是**真实测试工作**，但**不能**说"与夹具无关"。
    ...
    "pricelayer": Batch(
        ...
        # "慢批次取不超过上限的最大值"。
        # ★ 13-F 曾据 **127 例**的复测（33.52s / 33.46s）主张 180s = 5.4×；后 13-B 追加用例，
        #   取舍：**以 168 例的 83.80s 为基准**，180s 只有 **2.1×**、不成立 ⇒ **300s 保留**。
        # ★ 卡 13-F 的旧读数 **33.52s / 33.46s（当时的 127 例）已被取代**。
        #   ★★ **例数一律不写死**（`#91` 的政策）—— 它**一直在涨**，而且涨得很快：
        #      127 → 168 → 183（`#91` 当时现算）→ 192（`#102` 收尾，合并前）→ **201**（合并 `main` 后）。
        #      ⇒ 任何写死的数字**在提交之前就可能过期**（192 这个数就没活过这一轮）。**别把它当基准。**
        # ★ 本批**「夹具轻」、不是「0 夹具」**（主理人裁定 2026-09-16，卡 `#102`）：
        #   `tests/pricelayer/conftest.py` **自带 6 个 `@pytest.fixture`**（`scratch` 建最小工程根
        #   + 3 个写 jsonl 的夹具等），`scratch` 的 teardown 是
        #   `shutil.rmtree(root.parent, ignore_errors=True)`（≈10 次删除/例 ⇒ 轻，但不是**无**）。
        #   ⇒ ① **`D′`（`raw` 入 `_COPY_SKIP`）对它无效**：`raw` 是共享 `code_root` 里的子树，
        #        与它自带的树无关；② 守卫成本**本批没做过单变量 A/B**（6.2 只对 `unit` 做过）⇒
        #        **不得**拿 6.3 与 6.6 两次 `E` 读数之差（11.74 / 10.68 → 15.36）当守卫成本 ——
        #        那是**同配置重复测量**（`口径 12`，两轮都跑在 `E` 下）。
        #   ⇒ 别拿"夹具修复"整体去预期它变快，也**别**把它当"0 夹具"批看。
        # ★ 复测**已取得**（2026-09-16，**门禁口径 = 配置 `E`（守卫关）**，当时 **183 例**）：**`15.36s / 300s`，`exit=0`**
        #   （★ 例数只作**快照**；其后已涨到 **201**，见上 —— 不重测就不更新这个数，更不要拿它当基准。）
        #   ⇒ 300s = 19.5×（比 `V-02` 的 4~8× 宽），按"慢批次取不超过上限的最大值"保留。
        #   ★★ **不得把 83.80s → 15.36s 归因于 13-F 的夹具修复**（`口径 12`）：本批**不是"0 夹具"**
        #     （自带 6 个，见上），但 13-F 那条 `D′` 对它**无效**；且同期 `main` 删改过本批测试文件
        #     （用例数从 168 涨到 183 即其证据，此后仍在涨）。完整对照表见
        #     `reports/ws_fixture_cost_report.md §6.6`。
    ...
    "decision": Batch(
        ...
        #   ★ 本批**不依赖共享 `code_root` 大夹具，但 ≠「0 夹具」**（主理人裁定 2026-09-16，卡 `#102`）：
        #     `tests/decision/conftest.py` **自带 2 个 fixture**（`scratch` 建只含
        #     `facts/index/rules/derived/registry` 的最小工程根、`cli` 跑真子进程），
        #     `scratch` 的 teardown 是 `shutil.rmtree(root.parent, ignore_errors=True)`
        #     ⇒ 与 `compute`/`pricelayer` 同形：夹具**轻**，不是**无**；13-F 的 `D′` 对它**无效**。
        #     耗时主体仍是 97 例真实测试工作（现算复测 `2.39s/120s`，50×）。
        #   120s 对较慢一次 = **4.8×**，≤300s。
...
ORDER = (
    ...
    #   在某一处突然变红（`V-08`）。要完整覆盖该目录，请**按片轮流次**跑。
    "injection-a", "injection-b", "injection-c",
    "injection-d", "injection-e", "injection-f",
    "injection-g",
    "root",
