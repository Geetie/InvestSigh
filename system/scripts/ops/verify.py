...
BATCHES: Mapping[str, Batch] = {
    ...
    "guards": Batch(
        ...
        #
        # ★★ 超时 60s → 120s（卡 13-F / `G-54` 实测）：**60s 下本批每次都红** ——
        #   修前两次实测均 `exit=124`（`60.02s/60s` TIMEOUT，两次都超时！），因为 20 个夹具用例
        #   每例多付 ≈4.2s 的 `raw/` 清理（`tests/conftest.py::_reset_truth_source`）。
        #   修后两次实测：**27.13s / 19.11s** ⇒ 旧 60s 只剩 **2.2×**（< `V-02` 的 4×）⇒ 仍会随机红。
        #   取值 120s：对较慢一次 27.13s = **4.4×**，≤300s。
        "guards", "tests/guards/（门禁退出码契约 + 验证规范）",
        # ★ 超时 60s → **210s**（主理人裁定 · 批次 13 集成期）。
        #   13-F / `G-54` 实测：**修前**两次均 `exit=124`（`60.02s/60s`，即 60s 下本批**每次都红**）；
        #   **修后**（`raw/` 加入 `_COPY_SKIP`）两次 **27.13s / 19.11s**。
        #   ★ 13-F 自定 120s（= 4.4×），但**与它自己给注入分片定的规则不一致** ——
        #     它对分片用的是「**实测 × 8**（`V-02` 上沿）+ 向上取整到 30s 倍数 + 不低于 90s」，
        #     依据是它自己实测「**宿主并发可把同一片拖慢 11.8×**」。⇒ 4.4× 吸收不了并发。
        #   ⇒ 按它自己的规则：`27.13 × 8 = 217` ⇒ 取 **210s（7.7×，落在 `V-02` 的 4~8× 内且 ≤300s）**。
        #   ★ **残余风险（13-F 自测）**：并发可拖慢 11.8× ⇒ **没有任何 ≤300s 的上限是绝对安全的**；
        #     真正的解 = 13-F 的夹具降本 **+ 不并发跑批**；超时只是安全网。
        _pytest("tests/guards"), 210.0, _exit_zero,
    ),
