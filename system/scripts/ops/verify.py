...
| `injection-a`…`injection-f` | `tests/injection/` 的 **6 个分片**（每片一组显式文件路径） | 300s |
...
- 夹具是**每用例复制一份 `system/`**（`conftest.py::code_root`），**用完即删**；
  完整夹具（`copytree` + `_ENSURE_DIRS` + `_make_writable` + `_reset_truth_source`）实测每份
  **273 项**（裸 `copytree` 是 271 —— 两个数字都在本仓文档里出现过，**以 273 为准**：
  它才是"用完即删"要删掉的那个数）；
...
| 项 | 原依据 | 实测 |
|---|---|---|
| 计数单位 | "夹具项数"（271/例） | **≈792 计数/例**（≈2.9 × 273）—— 102,044 计数 ≈ 130 个建夹具用例 |
| 阈值 | 固定 `9999` | **两次观测不同**：主理人 `9999`、本单 `99999`（当前环境变量亦为 `99999`） |

⇒ 故 `32 × 271 = 8,672 < 9,999` **不能**用来论证"32 例是安全的"。
★★ 片大小取 **≤32** 的定位是「**实测可跑的当前配置**」，**不是**「已证明安全」：
① 32 是主理人在分片任务书里给的**授权上限**（**片数是主理人决策**，本单不擅自改）；
② 有**实测支持** —— 工作树内逐片 `exit=0`：a 27 例 · b 28 例 · d **31 例** · f 31 例
（d 是最接近上限的一片，也过了）。
★ **它成立的前提是阈值处于观测到的较高值（99999）。若阈值回落到 9999**，
按 `9999 ÷ 792 ≈ 12` 例/片重排（⇒ 完整覆盖需 **≈14 片**，即 **14 个轮次**）——
判据落点见 `tests/injection/test_shard_coverage.py::MAX_CASES_PER_SHARD` 上方注释
（该注释与断言的失败消息里都写着这三句，避免下一个人把它当推导常数）。
...
四条断言**各自都有反向对照**（`G-05`：拦截判据必须有反例，且必须是"真的会红"）——
原始输出见 `reports/ws_verify_shard_report.md §2.3`（独立复现见 §8.5）。
...
（禁令与"为什么这条没有机器绑定、且不可能有"的说明见 `INJECTION_SHARDS` 上方注释）。
...
#   `tests/injection` 共 165 例；每例约 **792 计数**（实测 ≈ 2.9 × 夹具项数 273）
#   ≈ **13 万计数/轮**，远超宿主单轮删除配额 ⇒ 原来那个目录全量批次**跑不完**。
#   完整覆盖该目录现在需要 **6 个轮次**（每轮一片）。
#
# ★★ **显式禁令：不许用 `--batch all` 覆盖 `tests/injection`。**
#   6 片连跑 = 又变成"一轮 165 例" = 配额问题**原样复发**，且红的是**第 N 片之后的某一片**，
#   看起来像"那一批坏了"。**要跑全，就分 6 个轮次，一轮一片。**
#   ★ 这条禁令**没有机器绑定，且不可能有**（如实说明，不假装有守卫）：
#     `--batch all` 的越界是**跨进程累积**的 —— 每一片**单跑都是绿的**，红的只是"前面已经删了太多"
#     的那一片。子进程无法知道自己之前删了多少；要在进程内拦它，就得让 `verify.py` 维护
#     "本轮已跑过哪些片"的**跨进程状态**，那是把一句纪律做成一整套状态机（过度设计，
#     且状态本身会漂移 —— 本仓已多次被"同一事实第二存放处"咬过，见 `G-06`）。
#   ⇒ **机器绑定管静态可分片性**（穷尽/不重/不超量/目标形态，4 条断言，见
#     `tests/injection/test_shard_coverage.py`）；**"一轮一片"是使用纪律**（`V-08`）。
#     两者分工不同，不要互相冒充。
...
BATCHES: Mapping[str, Batch] = {
    ...
    # ★ 超时：六片**统一 300s** = `V-02` 允许的上限（见 `CONVENTIONS.md::V-02` 批次表）。
    #   为什么不按"实测 × 倍"逐片取值 —— 三条实测教训（本单第一手）：
    #     ① **标定基准必须是工作树内实测**：同一份代码、同一片 `injection-f`，
    #        工作树内 **110.13s**、隔离副本 **12.53s**（8.8×）。差异来自宿主在工作区内
    #        **逐项监察删除**（实测 ≈16ms/项；同一份 273 项夹具 `rmtree` 工作区内 4.521s
    #        vs 工作区外 0.046s = **98×**），而 `copytree` 两边一样快。
    #        ⇒ **隔离副本会把最贵的那一项测成 0**。按隔离 ×8 得 120s，对 f 只剩 **1.09×**
    #        ⇒「天天误报的门禁一定会被关掉」（`G-01`）。
    #     ② `V-02` 的"慢批次 4~8×"在本目录**撞上限**：f 的 110.13s × 4 = 440s > 300s。
    #        与 `daily`（43s 实测 → 180s，4.2×）同一处境 ⇒ **取不超过上限的最大值**。
    #     ③ 300s 仍是**真的故障探测器**：对工作树实测 a 80.92s（3.7×）· b 63.72s（4.7×）·
    #        d 136.80s（2.2×）· f 110.13s（2.7×）。
    #     ★ 遗留风险（如实登记，不粉饰）：d（2.2×）与 f（2.7×）余量偏薄。若**同时**有第二个
    #       pytest 会话在同一工作树里跑（`V-05` 禁止的情形：实测曾把 a 从 7s 拖到 80.92s，
    #       `tests/.work/` 里留着别批的夹具残留），这两片仍可能被拖过 300s 而**假红**。
    #       ⇒ **见到这两片超时的第一步不是改断言，而是先确认有没有第二个会话在同一工作树里跑。**
    "injection-a": Batch(
    ...
    "injection-c": Batch(
        ...
        # 原文件 41 例 ≈ **3.2 万计数**（实测 ≈792 计数/例）**单文件就远超配额**，
        # 必须按 `# ═══` 分节边界拆两半（纯移动，断言语义零改动）。
