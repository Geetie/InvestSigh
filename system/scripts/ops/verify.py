...
BATCHES: Mapping[str, Batch] = {
    ...
    "pricelayer": Batch(
        ...
        # ★ 卡 13-F 的旧读数 **33.52s / 33.46s（127 例）已被取代**（用例数 127→168→**现算 183**）。
        # ★ 本批 **0 个夹具用例** ⇒ 卡 13-F 的夹具修复**不会**改善它（别拿修后去预期它变快）。
        # ★ 现算复测**已取得**（2026-09-16，生产配置 = 守卫开，`183 例`）：**`15.36s / 300s`，`exit=0`**
        #   ⇒ 300s = 19.5×（比 `V-02` 的 4~8× 宽），按"慢批次取不超过上限的最大值"保留。
        #   ★★ **不得把 83.80s → 15.36s 归因于 13-F 的夹具修复**（`口径 12`）：本批 0 夹具用例，
        #     且同期 `main` 删改过本批测试文件（用例数 168→183 即其证据）。完整对照表见
