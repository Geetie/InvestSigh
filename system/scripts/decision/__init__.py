...
| `step` | 对编排器的**接线接缝**（`StepHandler` 签名一致；`run_date` = as-of 上界、`scope` = 目标证券） | `Ch9 §3.5` 阶段④/⑤ |
| `run_decide` | **真实运行**入口（读**真源**行情 `facts/prices.jsonl` → `compute` 出 `DerivedValue` → 决策 → 落 `facts/recommendations.jsonl`） | `Ch7 §F` 端到端时序 |
