...
### §5.4 ★★ 我第 8 次仪器错：判"我的报告不在 `main`"，且**连续两次判错变量**

★ 本节序号按写入顺序（时间顺序 §5.1 → §5.4 → §5.3）。

`main` 前进到 `3d200ac` 后我复核"本卡产出有没有被吸收"，第一版读数：

```
$ git ls-tree -r --name-only 3d200ac -- system/reports/ | grep -i 'card91\|shard_drift'
$ echo "(空=不在 main)"
(空=不在 main)                     ← ★★ 假零 ★★（0 命中）
```

**同一份输入**（同一个 `ls-tree`，同一个管道），换一种写法就命中：

```
$ git ls-tree -r --name-only 3d200ac -- system/reports/ | grep -i -E 'card|91|shard'
system/reports/ws_card91_shard_drift_report.md      ← ★ 报告一直在 main 里
system/reports/ws_verify_shard_report.md
```

**★ 两次错误归因（本节重点 —— 错的是归因，不是读数）**

| 次序 | 我的假设 | 为什么错 |
|---|---|---|
| ① | `\|` 在 **BSD grep** 上失效 | **错**：`/usr/bin/grep` 自报 `grep (BSD grep, GNU compatible) 2.6.0-FreeBSD`，`\|` **有效** |
| ② | 变量是"**文件参数 vs 管道**" | **错**：那次对比**同时换了仪器** ⇒ 混杂（confounded）。"文件"那格我用的是 `/usr/bin/grep`，"管道"那格是**裸 `grep`** |

**正交叉 2×2（唯一变量 = 仪器；模式固定 `aaa\|bbb`；输入 `/tmp/m91/t.txt`）**：

```
bin=grep           src=file rc=1 out=[]              ← 裸 grep = toybox 垫片
bin=grep           src=pipe rc=1 out=[]
bin=/usr/bin/grep  src=file rc=0 out=[aaa,bbb,]
bin=/usr/bin/grep  src=pipe rc=0 out=[aaa,bbb,]
```

⇒ **输入源（文件 / 管道）根本不是变量**：两条仪器各自在两种输入源上**行为一致**。

**仪器指纹（`V-10` 规则 1）—— 一行就能定性，我两次都栽在没先读这一行**：

```
$ command -v grep
…/cli/vendor/shim/brokered-bin/grep -> codebuddy-toybox-dispatch      （symlink）
$ grep --version
toybox 0.8.13 (is not GNU grep 9.0)                                   （垫片）
$ /usr/bin/grep --version
grep (BSD grep, GNU compatible) 2.6.0-FreeBSD                         （真 BSD）
```

**失效集逐项**（输入 `alpha beta` / `axb`；★ 末行是**控制组**）：

| 模式 | 裸 `grep`（toybox 0.8.13） | `/usr/bin/grep`（BSD 2.6.0） |
|---|---|---|
| `alpha\|beta` | `rc=1` **零命中** | `rc=0` 命中 1 |
| `a\+lpha` | `rc=1` **零命中** | `rc=0` 命中 1 |
| `alpha\?` | `rc=1` **零命中** | `rc=0` 命中 1 |
| `alpha\>` | `rc=1` **零命中** | `rc=0` 命中 1 |
| `\balpha` | `rc=1` **零命中** | `rc=0` 命中 1 |
| `a\<lpha` | `rc=1` **零命中** | `rc=1` **零命中** ← 唯一在 BSD 也失效的 |
| `alpha`（**控制组**） | `rc=0` 命中 1 | `rc=0` 命中 1 |

★★ **三条新事实（均可复跑）**：
1. **toybox 垫片的失效集 6/6 全中**（`\|` `\+` `\?` `\<` `\>` `\b`）—— 与 `#82` §5 / `#98` 已登记的一致。
2. ★ **BSD grep 只失效 `\<` 一项**（`\|` `\+` `\?` `\>` `\b` 全部有效）⇒ ★ **"某写法失效"的结论不能从垫片外推到系统同名命令，反之亦然**。（这条正是我 `#82` 报告里那处"BSD 误归因"的**同一误归因的第二次** —— 只是这次它落在了"垫片 vs BSD"的方向上。）
3. **控制组（单模式）在同一条管道上立刻命中** ⇒ 假零**自我证伪**的成本只有一行。

★ **教训（建议进 `V-10`，与 §9.1、§5.1 同族）**：
- **(a) 一次只改一个变量**：我两次"定位根因"各只跑了 **2 格**（2 变量至少要 4 格，这里是 2 仪器 × 2 输入源 × 2 写法 = **8 格**）。**把"仪器"和"输入源"当成同一个变量，是最容易犯的混杂。**
- **(b) 报"某写法失效"必须带仪器指纹**：`grep --version` 一行就能区分 toybox / BSD / GNU。**不含仪器指纹的"失效集"结论，举证半径只有那一台仪器。**
- **(c) 零命中必须配控制组**（`V-10` 规则 3）：本次控制组 30 秒内把假零推翻。

★ 这是**我第 8 次仪器错**（前 7 次见 `#82` 报告 §7.3 与本文 §5.1 的逐次留痕）。★ 值得注意的是**它和 §5.1 相隔不到两小时，而两次的根因是同一句话**：**没有先读仪器指纹、没有先配控制组**。⇒ `V-10` 规则 1（仪器指纹）与规则 3（控制组）不是形式主义，是**本次会话命中率最高的两条**。
...
★★ **上表绑定 `main@b1000b4`。`3d200ac` 上的重判见 §9.6，已变化**：`test_shard_coverage.py` 那行**已被 `main` 吸收**（`ab940dc`，与我的 `41fbe7e` **逐字节相同**）；`verify.py` 四行中 **L29 / L72 附近的措辞 `main` 已自己真源化**，而 **`≈14 片` 两处仍未修**。
...
### §9.6 `main@3d200ac` 复核（第二次；**不复用 `b1000b4` 的任何读数**）

`main` 在我提交后已从 `b1000b4` 前进到 `3d200ac`（含 `a889af6` / `a797b28` / `e17d743` 等，以及**我 4 个提交的 rebase 副本**）。

#### ① `main` 已吸收我 **4** 个提交（含报告本体）—— 逐字节可证

| 我的 | `main` | 内容 | 证据 |
|---|---|---|---|
| `89c6e96` | `74e2321` | 交付报告（351 行版） | `git show --stat 74e2321` ⇒ `ws_card91_shard_drift_report.md \| 351 +` |
| `420800e` | `d1df713` | §0.1 撤回与并列声明 | 同上（`--stat` 各一条） |
| `7bab705` | `f70cf45` | §9.4 行号表 | 同上 |
| `41fbe7e` | `ab940dc` | `test_shard_coverage.py` 4 处 | `git diff ab940dc:<f> 41fbe7e:<f>` **空**（逐字节相同） |

**未吸收 3 个**：`1706916`（`verify.py` 16 处）、`6f71a3c`（`CONVENTIONS.md` 10 处）、`db5977b`（本报告 §5.3）。
→ `main` 树里的本报告 = **497 行**，我版 = **517 行**，差 **`27+ / 7−`**，**恰好只差 `db5977b`**（`git diff --stat 3d200ac:<f> db5977b:<f>`）。

★ ⇒ **§0.1 的取舍建议在 `3d200ac` 上仍成立**，且 **`db5977b` 是唯一还缺的一节**（副作用披露）。**我不擅自往 `main` 推**，等裁定。

#### ② 我那两个"内容提交"确认为**重复劳动** —— `main` 已用**自己的措辞**做了同一件事，且**明文引用 `#91`**

```
main:verify.py:90         （穷尽性 · 两两不相交 · 每片用例数 `--collect-only` 现算 ≤ 32 · 目标必须是显式测试文件路径）
main:verify.py:92-93      ★ 代价…需要**与 `INJECTION_SHARDS` 片数相同的轮次数**（每轮跑一片 —— 片数不在此处写死，真源就是那个常量）
main:verify.py:250-253    ★ 片数与各片例数一律不在此处写死：唯一真源 = `INJECTION_SHARDS` 常量，例数用**现算**值
main:verify.py:426        —— 本卡（#91）修的就是"把当时的例数写成永恒事实"这一类漂移。
main:verify.py:490        ★★ **例数一律不写死**（`#91` 的政策）—— 它**一直在涨**
main:verify.py:602        各片必须**逐片单独跑**（见 `INJECTION_SHARDS` 上方的理由）
main:CONVENTIONS.md:92-97 ★★ **本表刻意**不写「片数」与「例数」（`#91`…唯一真源 = …）
main:CONVENTIONS.md:169   ★ **片数与各片例数一律不在此处写死**（`#91`）
```

★ **并且**：`CONVENTIONS.md` 里的 `183` / `168` / `192` 在 `3d200ac` 上**已全部消失**（`main` 的 `98af920`「§③ 改为指真源」+ `4e70014`「数字加『当时读数』限定词」）⇒ **§9.3 登记的那条在 `main` 上已被闭合** —— **不是我修的，是 `main` 自己修的**。★ 我的 `6f71a3c`（`CONVENTIONS.md` 10 处）此刻若合入，效果是**把这批新做的"当时读数"限定往回退**（§9.2 已量：净 **259 行 = 回退**）。

#### ③ ★ 但 `main@3d200ac` 仍有 **3 处**同族残留（`≈14 片`）；真值 **≈17** —— 本卡范围内 `main` 内的**真实缺口**

| 位置 | 原文（`3d200ac`） | 现算 |
|---|---|---|
| `verify.py:73` | 「按 `9999 ÷ 785 ≈ 12` 例/片重排（**≈14 片**）——」 | `⌈205 ÷ 12⌉ = 18`（`205 ÷ 12 ≈ 17.1`） |
| `verify.py:262` | 「现登记的片数（每片 ≤32 例）在 **9,999** 这个较低阈值下**不成立**（需 **≈14 片**）；」 | 同上 |
| `CONVENTIONS.md:169` | 「按 `9999 ÷ 792`，安全上限只剩 **≈12 例/片**，完整覆盖需 **≈14 片**。」 | 同上 |
| `CONVENTIONS.md:172` | 「（安全上限 ≈12 例/片 ⇒ 需 **≈14 片**）」 | 同上 |

`14` 的来源是 `165 ÷ 12 ≈ 13.75` / `171 ÷ 12 = 14.25`（**当年的**例数）—— ★ 与 §9.3 **同一根因**：把"当时读数"写成了常量。
★ 两处的**邻行**都已改成"指向真源"了（`verify.py:173-175` 明说"原文此处曾写「6 片 / 171 例」…实测已全部漂移"）⇒ **唯独 `≈14 片` 这一句漏了**，正是"逐行改、却漏掉跨行推理里的那个数"的典型形态。

**⇒ 建议**（指派属主，**我不动**：`verify.py` 归 `ws-verify-shard`〔他在做 `verify.py` 真源化〕；`CONVENTIONS.md::V-02` 归 `ws-ch2-rules`〔V-02 规则落纸人〕）：把上表 4 处一并真源化，写法**抄同一段里已改好的那句**（"按当次现算例数 ÷ 12"）。

★ **边界**：`≈12 例/片` 自身依赖 `792`（vs `verify.py` 的 `785`，§6.3 **未裁**）与阈值 `9999`（**非常数**，§6.4）⇒ 我**只报"`14` 与现算例数不符"**，**不代裁**换算口径。

#### ④ ★ `main` 的合计例数 **≠** 我的 `205` —— 我**不代读**

`main:CONVENTIONS.md:100` 自述「`injection-b` 已 `32 → 34`」，与任务列表 `#109`（`injection-b 34 > 32` 越限）一致 ⇒ `main` 的合计按此应为 **207**（他处未变的前提下）。
★ 我在 `3d200ac` 这个 revision 上**没有起过 pytest 会话**（本卡交付后我已向 `ws-real-collect-2` 承诺零负载）⇒ **不报 `main` 的逐片例数**（`V-11` 举证半径）。**`205` 只是我树 `db5977b` 上的现算值**，别把它当 `main` 的值用。

#### ⑤ 复跑命令（全部只读、不依赖我的分支）

```
git show --stat 74e2321 d1df713 f70cf45 ab940dc                      # ① 我 4 个提交的 rebase 副本
git diff --stat ab940dc:system/tests/injection/test_shard_coverage.py \
               41fbe7e:system/tests/injection/test_shard_coverage.py  # ① 逐字节相同（应无输出）
git diff --stat 3d200ac:system/reports/ws_card91_shard_drift_report.md \
               db5977b:system/reports/ws_card91_shard_drift_report.md  # ① 只差 §5.3（27+/7−）
git show 3d200ac:system/scripts/ops/verify.py | grep -nE '≈14 片|#91'  # ③ 残留 vs 已改好
git show 3d200ac:system/CONVENTIONS.md | grep -nE '≈14 片|183|168'     # ③ 残留；183/168 应为空
git show 3d200ac:system/CONVENTIONS.md | grep -nE '32 → 34'            # ④ main 自述的例数变化
```
