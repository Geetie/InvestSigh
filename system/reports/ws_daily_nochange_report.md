...
### 2.6 ★ 按 `CONVENTIONS.md::V-09` 复跑尝试：**结论 = 当前回合不可恢复**（附守卫的精确模型）
...
**（b）★ 守卫的判定规则（实测反推出来的精确模型）**

观测到的拒绝报文（自建探针，9 次，只摘 `count` 与目标体量）：
...
⇒ 拟合：**`count = 本回合已累计 + 本次目标体量`，当且仅当 `count > threshold(99999)` 时拒绝**。
本回合的**已累计 ≈ 99,998**（= 99999 − 1，**正好压在天花板上**）。

**（c）由此得出的两条硬结论**

1. **"压缩单次删除量"在本回合**在原理上**无效**：当已累计已在天花板时，
   `99998 + 1 = 99999` 都不越线、`99998 + 2` 就越线 —— 实测"删 1 项"已**被拒**
   （上表首行 `count` 恰为 `99999`，是边界值）。
2. **它解释了我早先那次"1 项/50 项通过、250 项被拒"的观测**：当时回合基数**恰好**低于天花板，
   故 `base+1`、`base+50` 未越线而 `base+250` 越线。
   ⇒ ★ **那次观测不能被读成"分片可行"**，特此更正以免误导（分片只在"基数低于天花板"时有窗口，
   而基数是**全流共享**的，我无法控制）。
...
| 10 | **回归分片**（`tests/daily` / `tests/guards` / injection 相关片） | — | ★ **未跑成**（宿主共享删除预算见底，§④-2）；已按 `V-09` 试过单文件/单用例/**0 夹具**压缩体量 ⇒ 全被拒 ⇒ **当前回合不可恢复**；替代证据见 §2.5、守卫精确模型见 §2.6；**如实登记为"未跑"而非"已验证"** |
...
- 实测反推出守卫规则：**`count = 本回合已累计 + 本次目标体量`，超过 `99999` 即拒**，
  本回合已累计 ≈ **99,998**（正好在天花板上）⇒ **"删 1 项"也被拒** ⇒ 压缩体量**在原理上无效**；
