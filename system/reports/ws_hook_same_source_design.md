...
## 11. ★★ 安装的覆盖面代价（**实测，装在动手之前**）—— 这是本卡唯一一条"装了会变差"的事实

§6 只说了"落后树少跑新门禁"这一定性。装之前我把**数量**量出来了（只读：读各工作树磁盘上的 `pre-commit.sh`）：

```
main 的门禁数 = 14
各工作树磁盘上 pre-commit.sh 的门禁条数分布（worktree 总数 32）：
    8 道 × 7 棵   10 道 × 10 棵   11 道 × 4 棵   13 道 × 1 棵   14 道 × 9 棵（含主仓）
    1 棵（tmp-head-check）无该文件
⇒ ★ **20 / 32 棵树的清单比 main 短**（最多短 6 道）。另：`is-ancestor main <sha>` 为假的有 **30 / 32**。
```

**⇒ 安装会造成一个可测的方向改变**：

| | 旧钩子（清单取主仓 / 脚本 cwd） | 新钩子（清单与脚本都取被检树） |
|---|---|---|
| 落后树提交 | 清单里那 6 道脚本**不在**树里 ⇒ `[Errno 2]` / `rc=2` ⇒ **提交被拒**（但被误报成"你改坏了"） | 清单短 ⇒ 只跑自己那 8/10/11/13 道 ⇒ **提交放行** |
| 那 6 道判据**实际跑了没有** | **没有**（只是"拒了提交"） | **没有**（不拒，也不再提） |

★ **所以这不是"覆盖面减少 6 道"** —— 那 6 道在落后树上**本来就跑不了**（脚本不存在）。
真正的改变是：**"拒了但没说清" → "放行了但更少门禁"**。
★ **集成线不受影响**：内容要进 `main`，必须在 `main` 根下再跑一遍 ⇒ 那时用的是 `main` 的 14 道清单
（`--show-toplevel` = 主仓根）。**这是采纳方案 A 的前提**，也是 §6 的论证。

**我的建议（把"静默少跑"变成"可见少跑"）**：安装的同时采纳 `§9.2-3` 那条 note ——
`pre-commit.sh` 开头打印**一行**：
```
pre-commit: 判据树 HEAD=<sha> · 本次门禁 <N> 道
```
⇒ 它是**同源安全**的（只报自己），却让"本次只跑了 8 道"**直接留在提交输出里**，
任何人拿 `main` 的 14 一比就知道落后了。**不新增门禁、不改判据**（遵 `G-07`）。

**★ 我没有把这个代价说成"可接受"就完事** —— 它是一条**真实的、量化的**取舍，
所以我把数字摆在这里，由 team-lead 定。若他认为"落后树必须被堵"更重要，
那么**正确的做法不是退回旧钩子**（那会用**假指控**去堵），而是**另立一条"树过期即拒"的显式判据**
（那属于 `G-07` 的新门禁，不属本卡）。

### 11.1 顺带发现（**不属本卡，未处理，仅登记**）

`git merge --ff-only <branch>` 是**快进 ⇒ 不产生提交 ⇒ `pre-commit` 根本不跑**。
⇒ 若集成走的是快进，则"集成线必然跑 `main` 的清单"这个前提**在快进路径上不成立**。
这与 13-O **无关**（旧钩子同样不跑），但它是本卡论证所依赖的那条前提的一个**真实缺口**。
我**未去核**本仓实际集成方式（不属本卡）⇒ 按 `G-03` **不计为已验证**，只作登记。

## 12. ★★ 安装记录（已执行，含**一条回滚命令**）

**授权**：主理人裁定 `e9a009b`（`system/reports/batch13_taskbook.md`「裁定 · `13-O` go —— 批准安装」，
宣布了安装窗口 + **三条前置**）。本节按前置逐条结账。

| 前置（裁定原文） | 我的执行 | 证据 |
|---|---|---|
| ① **必须在"无并发写入方"的窗口内安装**，且**安装 + 自检 + 验证全部在一次工具调用内完成** | 一次调用内跑完 A–F 六段 | 见下 |
| ② **装完立刻验证**：`--check` 必须返 `0`；并在**本树**与**主仓**两条路径上各跑一次 `pre-commit.sh`（不改状态） | 两条路径都跑，均 `rc=0`、14 PASS、阻断 0 | 见下 |
| ③ ★ **`.bak` 保留到本轮结束**，并在**报告里给出**一条回滚命令 | 本节即为该处 | 见下 |

### 12.1 安装前后（共享钩子 = **全 34 人共享**）

| | 值 |
|---|---|
| 对象 | `/Users/gaza/Developer/InvestSigh/.git/hooks/pre-commit` |
| 安装前 | **138 bytes** · mtime `2026-09-16 00:13:17` · sha256 `82e66123d6b1309c5c73d39454b9651393f627c23b6854f65a41cf509ee3ac15`（正文 = `exec sh "/Users/…/InvestSigh/system/scripts/ops/pre-commit.sh"`） |
| 备份 | `/Users/gaza/Developer/InvestSigh/.git/hooks/pre-commit.bak.20260916232650`（**保留到本轮结束**） |
| 安装后 | sha256 `5e18ac0586d22c6bc16b761ad9014ee5e73d70043b3e03766982ed4134059a43` |
| 安装时刻 | `2026-09-16 23:26:50`（备份文件名即时刻） |

### 12.2 ★ 一条回滚命令（裁定前置 ③ 要求）

```sh
cp '/Users/gaza/Developer/InvestSigh/.git/hooks/pre-commit.bak.20260916232650' '/Users/gaza/Developer/InvestSigh/.git/hooks/pre-commit' && chmod +x '/Users/gaza/Developer/InvestSigh/.git/hooks/pre-commit'
```

（安装脚本自己也会把这行打出来 —— 见 `install_hooks.sh` 的备份分支。）

### 12.3 验证（前置 ②）

| # | 项 | 结果 |
|---|---|---|
| 1 | `sh install_hooks.sh --check` | **`rc=0`** —— `已安装的薄壳 == 版本化模板` |
| 2 | **本树**经**已安装钩子**跑一次（`sh <钩子>`，cwd = 本树） | `rc=0` · 14 PASS · 阻断 **0** · INPUT-ERROR **0**<br>★ 首行：`判据=/Users/gaza/…/.worktrees/ws-ch2-rules/system/scripts/ops/pre-commit.sh` ⇒ **T1 同源在真钩子上成立**（解析到**本树**，不是主仓） |
| 3 | **主仓**经**同一个已安装钩子**跑一次（cwd = 主仓） | `rc=0` · 14 PASS · 阻断 **0**<br>★ 判据 = **主仓自己**的 `pre-commit.sh` ⇒ **主仓行为不变**（注：主仓那棵树的脚本还是旧版，所以它的首行**没有** HEAD 提示 —— 这正是同源：**每棵树用自己那份脚本**） |
| 4 | ★★ **首次真实 `git commit` 走新钩子** | `rc=0`，提交 `97fff56` 成功，首行 `判据树 HEAD=66df243 · 本树门禁 14 道 · 判据=<本树路径>`，14 PASS |

★ 第 4 条是**最强的那条证据**：它不是在测试里、也不是手工调用，而是**一次真实提交**，
  在**真实的共享钩子**上，把"判据来自被检的那棵树"**印在了输出里**。

### 12.4 安装后的行为变化（**要如实告知全队**）

| 方向 | 旧钩子 | 新钩子 |
|---|---|---|
| ① 判据**新**于对象 | `[Errno 2]` / `rc=2` / **被误报成"阻断"**（假指控；本批次 3 次、≥2 条流） | **不再出现**（清单与脚本同源）；若清单脚本真缺失 ⇒ `[INPUT-ERROR]` + `exit 2`，**不写"阻断"** |
| ② 判据**旧**于对象 | 静默少跑门禁、**零报错**（完全不可观测） | **仍然少跑**（设计取舍，见 §6/§11），但**现在可见**：每次提交打印 `判据树 HEAD=… · 本树门禁 N 道` |
| 落后树（实测 20/32 棵清单比 `main` 短） | 提交被**拒**（但理由是假的） | 提交**放行**，且**明示**自己只跑了 8/10/11/13 道 |

★ 这就是 §11 量出的那条取舍的**落地**；它**不是**"覆盖面掉了 6 道"（那 6 道在落后树上本来就跑不了），
  而是"**拒了但没说清**"→"**放行了但明说跑了几道**"。

### 12.5 本次安装的残余与边界（如实）

1. **本卡不含任何耗时/配额读数**（`口径 21`：无跑批测量，故无需声明仪器）。
2. 方向②**仍未实测**（未造出"判据旧于对象"的现场）⇒ 按 `G-03` **不计为已验证**；机制由同一变量决定（§8.4 已证）。
3. 残余 **R1**（§10-2）：被检树可以改自己那份 `pre-commit.sh` 放行自己 —— 未变，仍非机器强制点。
4. `.bak` **保留到本轮结束**（前置 ③）；本轮结束后是否清理**由主理人定**（我未自作处置）。
