...
| 4 | **tdx-connector**（通达信） | ✅ **已入册**（2026-09-17 按 `N6.1-13` 补登记，见 §五） | ✅ 已连 | ✅ **实测覆盖面最广**：美股行情（`DelayHQMin:15`）· K 线**三口径可区分** · **公司行动表（分红/拆股）** |
...
| **缺口 2** | **② tdx 入册**（走 `N6.1-13` 变更审批） | ✅ tdx 实测有（AGIX 2 条分红 / NVDA 12 条 + 1 条拆股） | ✅ **已执行（2026-09-17 15:4x，你已批准）** —— 见 §五 更新说明 |
...
> ★★ **2026-09-17 15:4x 更新：`tdx` 已按 `N6.1-13` 变更审批补入白名单。**
> 改动面：`rules/data-sources.allowlist.yaml`（0444 已重锁）· `registry/launch_criteria.yaml`（`required` 与
> `available_sources` 各加一项）· `registry/sources.yaml`（新增 `src_tdx_mcp` 的 7 字段登记）·
> `registry/rules.lock.json`（重锁，14 文件）· `scripts/checks/launch_guard.py`（见下）。
> ⇒ **本节标题里的"不在册"对 `tdx` 已不成立**；下面表格保留，作为它当时为何该入册的实测依据。
>
> ★ 顺带修掉一条**同族缺陷**：`launch_guard` 原先写死 `if s.get("id") not in D01_SOURCE_IDS: continue`
> （只校验 `iFinD`/`pandadata`）—— **`tdx` 入册后它的许可字段将无人校验**（又一次「有声明、无消费者」）。
> 已加第 ④ 条判据：`available_sources` **每一项**都须 `license_tier` 可判定、`free_quota` 项 `cost` 必须为 0、
> 付费源不得出现在该段。并**新补 `tests/guards/test_launch_guard.py`（10 条，此前该守卫零测试覆盖）**，
> 其中两条专门证伪旧写法：拿 `git show HEAD:` 的旧版本喂同一份输入 ⇒ **旧版 0 违例（漏检）、新版 1 违例（拦下）**。
> ★ 另有一条**自证**：我初版往白名单条目里加了 `approved_at` / `approval_anchor` / `evidence_ref` 三个键，
> **`rule_key_alignment_guard` 立刻报 3 条 FATAL**（"在机器上零效力"）。处置是**删键、依据留在文件头注释**，
> 而不是登记豁免 —— 与 `westock`/`iFinD`/`pandadata` 保持逐字段同构。
...
1. **`launch_guard` 不探活**：它只校验 YAML 里的**纸面声明**（`license_tier=free_quota` / `cost=0`），
   **不校验数据源实际可达**。⇒ pandadata 现在"声明免费额度、实际无权限"，**29 道门禁全绿、无一报红**。
   ...
   ★ 已修的部分（2026-09-17）：**覆盖面**从"只认 `iFinD`/`pandadata` 两项"扩到
   `available_sources` 的**每一项**（另见 §五 更新说明与新增的 10 条测试）。
   ★ **仍未修的是"探活"** —— 那需要守卫去调外部端点，超出静态守卫的职责边界；
   故"实际可达"这件事**只能靠人实测**（本报告做的就是这件事）。**不要**把它写成"门禁已覆盖"。
2. ~~**`fq` 参数不可区分 + 分红再投资无数据**~~ → ★ **已解决**：
   复权口径改用 **iFinD**（可指定后复权）＋**tdx**（`tqFlag` 三口径可区分）；
   公司行动（分红·拆股）改用 **tdx**（已入册）。详见 §二 的两处"★★ 解法"。
...
| 优先级 | 动作 | 性质 | 阻断谁 |
|---|---|---|---|
| ✅ **已完成** | **缺口 1**：iFinD 取复权序列（在册源，无审批） | 已验证 | 缺口 1 |
| ✅ **已完成** | **缺口 2**：`tdx` 按 `N6.1-13` 补入白名单（+ 三处同步 + 守卫加强 + 10 条新测试） | 已执行 | 缺口 2 |
| **P0** | ★ **写"行情/基准 → `facts/prices.jsonl` / `benchmarks.jsonl`"落库脚本** —— 三源数据均验证可取，**这是唯一剩下的技术堵点** | 我做 | ★ **`batch 0`**；步骤 5/6 的 4 条 gap |
| **P0** | 拍两件事即可开工（见 `test_run_sheet_batched.md §0.5`）：① 基准身份用建议值 `AGIX`？② `p10::caliber_terms` 保持 `tbd`？ | **你确认** | 落库脚本的字段取值 |
| **P1** | 落库时用 iFinD 补 AGIX 的**净值 `45.03`** 与**跟踪指数名**（westock 空壳那两项） | 我做 | `benchmarks` 覆盖率字段 |
| **P1** | 向 pandadata 侧确认账号为何**整体无数据权限**（连交易日历都拒）→ 开不了则按 `N9.3-07` 如实标注该来源"不可用" | 你去问 / 我改登记 | `N9.3-07` 备选来源 |
| — | 不动 `agent-mail` / `github` | — | — |

### 一句话（15:5x 最终）

> **数据源侧已全部就绪**：`westock` ✅ · `iFinD` ✅（已购月度账号）· `tdx` ✅（已入册）· `pandadata` ❌（无数据权限，不影响主流程）。
> **两个缺口都已解决**：复权口径 → iFinD；公司行动 → tdx。
> **唯一剩下的技术堵点是落库脚本** —— 没有它，`prices` / `benchmarks` 永远是 0 行，
> 步骤 5/6 的 4 条 gap 与 `blocked=True` 也永远不变（**已实测：tdx 入册后 12 条 gap 逐字未变**）。
> 但它**只差"写"**：数据三源都能取、口径有解、表结构确定、写入通道（`append_records`）现成。
