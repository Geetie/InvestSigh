...
8. **★ 取证工具本身有坑**（§7.3）：Bash 工具里的 `grep` 是 broker 包装器，**对 GNU 扩展运算符 `\|` `\+` `\?` `\<` `\>` 会静默返回空（`rc=1`）**；而 **POSIX BRE（`\.` `\{n,m\}` `\(\)` `\1`）与纯字面是准的**（§7.3.1 逐格有真值）。我在本次审计里中过两次（其中一次"结论对、证据假"）。此后一律用 Grep 工具 / `/usr/bin/grep` / Python。**§3.2/§3.3 的数字不来自 greps**（一个来自断言消息、一个来自 pytest 执行），故不受影响。
...
1. **"纯字面**且**确有匹配"这一格**：裸 `grep` **返回 2 = 真值，完全正确**。
   ⇒ **`ws-verify-shard` 原来的"裸 grep 静默返回空、连删 1 项都被拒"**那种一般化表述 **在我这里无法复现**：我用**与他报告完全相同的命令形状**试了 ——
   ```
   $ <裸grep> -c "_safe_shutil_rmtree" <含 4 次命中 的文件>   →  '4\n'  rc=0   ✅ 正确
   $ /usr/bin/grep -c "_safe_shutil_rmtree" <同一文件>          →  '4\n'  rc=0   ✅ 一致
   ```
   ⇒ 他的观察**我复现不出来**。三个候选解释（**我不下判词**）：(a) 他当时那次读数的**输入与现在不同**（文件/路径/管道消费输出）；(b) 宿主 shim **在那之后变了**（它是 broker 中介的，可能有状态）；(c) 他那个 `0` 是**真的 0**（模式确实不在那个文件里）而被读成"工具坏了" —— 正是他自己反省的"零命中三候选未做区分实验"。
   ⇒ **他的操作建议（改用 `/usr/bin/grep`）依然正确**，但"**裸 grep 一概不可信**"这个结论**太宽**，且**已进入全流口径**，所以我据实测把它收窄。
2. **`-E` 并非无条件安全**：`-E '(a)\1'`（ERE 反向引用）在**裸 grep 上 0（真值 2）**，在 **`/usr/bin/grep` 上 2（正确）** ⇒ **"改用 `-E`"只在 `/usr/bin/grep` 下成立**；裸 grep 的 `-E` 仍会静默丢反向引用。
