...
| 2 | `P-05` 的 0.5s 对**所有读真源的守卫**都不达标（含既有 `locator_check`） | 建议把 `P-05` 的口径改为"**守卫自身扫描耗时** < 0.5s（不含解释器与共享依赖导入）"，或单独立项解耦 `schema.store` 的 pydantic 导入 |
...
| 7 | ★ **新增 `raw/` 抓取物会让 `code_root` 夹具命中宿主 safe-delete 配额**（见 §12） | 建议把 `"raw"` 加进 `conftest._COPY_SKIP` |
| 8 | ★ **同一 worktree 有并发写者**（见 §7.6） | 建议确认并只保留一个执行体后再合并 |
...
- 效果：每个用例少 18 次 `unlink`、少复制 281 KB ⇒ 既更快，也不碰那个 fail-closed 的删除路径。
...
**夹具不该继承真仓库的数据量**。本批次把真仓库的数据量提高了 4.5 倍，
把这个原本潜伏的耦合推到了显形阈值之上。
