...
### 13.12 ★★ **撤回：「`_clear_work_dir()` 改逐文件删除」是错的方向**（`ws-ch2-rules` 论证，主理人采纳）

**主理人先前把它写成 `G-60` 的"唯一能救的方向"并广播给多条流 —— 该判断错误，现撤回。**

设阈值 `T = 99,999`、判据 `base + 本次体量 >= T ⇒ 拒`：
1. **粒度不改变预算**：一次删 1 项与一次删 269 项，**总共都只能删到 `base + 体量 < T`**；
2. **当下 `base = 99,998`** ⇒ 单次删 1 项即 `99,999 >= T` ⇒ **逐文件连第一项都过不去**；
3. ★★ **更糟（新失败形态）**：当 `base < T` 但 `base + 269 >= T` ——
   **批量删** = **一次原子拒绝（夹具原封不动）**；
   **逐文件** = **先删掉前半、然后开始被拒** ⇒ **夹具半清空** ⇒ `pytest` 只看见一个 `E`。
   ⇒ 这是「**静默变空**」家族的**第四种形态**：「**清了一半**」与「**清干净了**」**观测上不可区分**，
   下一条用例拿到**脏且残缺**的夹具 ⇒ **比完全没清更容易造假绿/假红**。

⇒ **`G-60` 的处置栏据此改写**：**不是**"逐文件删除"，而是下面三条**验收判据**（`ws-ch2-rules` 提出，主理人采纳）：
1. **单次删除目标体量 ≤ 1**（只作为"降噪"，**不是**解药）；
2. **`base < T` 时一次会话可全程跑完**（真正的目标）；
3. ★ **部分删除之后必须响亮失败、且夹具状态可判定**（防上面那个**静默半清空通过验收却无人发现**）。

### ★ 备选出路（**待最小可行性实验验证**）：把夹具工作目录放到**系统临时目录**
`ws-ch2-rules` 实测：**`/tmp` 是独立低基数桶（4,334）且不受该守卫拦**；它的 §17.5 探针在 `/tmp` 下**完整跑完 A–F**，
而**同一回合**仓库内整文件回归是 `27 E`。
⇒ **若成立，本会话即可全程跑完夹具批次**（不必等新回合、不必求授权）。
★ **但主理人的项目记忆里有一条反证**：「**夹具临时目录必须在工作区内且是点目录**（`system/tests/.work/`）
—— 工作区外 `mkdir` 走 broker → `PermissionError`」。⇒ **必须实测，不许假设**（它那条观测可能已过期，或与 broker 配置有关）。
★ **代价（设计取舍，不是纯收益）**：夹具落到 `/tmp` ⇒ **失去"仓库内可追溯、被 `.gitignore` 管理"的性质**；
且 **`/tmp` 有独立的系统清理**（会话结束后可能被回收）。
★ **是否算"绕过"**：**不算** —— 该守卫**只管仓库内的删除**；把**一次性临时工作**放在它的**管辖域之外**是正常用法，
**不是**关闸门（我们**没有**动 `--no-report` / 环境变量 / `_clear_work_dir`）。

### 判据缺陷（`ws-ch2-rules` 提出，采纳）：**报文里读不出"被删条数"**
宿主报文给的字段是 **`targetCount` = 目标个数**（1 个目录就是 1），**"被删条目数"不在报文里**
⇒ `count = 基数 + 本次目标体量` 这个式子里的"**体量**"**从日志读不出来** ⇒ **归属只能靠猜**。
⇒ 登记：**引用该守卫的模型时，必须先验明每个字段的语义**（`targetCount` ≠ 条目数）；
并请宿主**同时打印 base 与体量**（否则"谁的请求把 count 推上去"永远无法定案）。

### ★★ 手法采纳（**配额耗尽期的标准审计范式**）：`--noconftest` + `--collect-only`
`ws-step56-skipped` 在 `G-60` 噪声期完成了一次**零删除**的判据验证：
```bash
cd <树>/system && PYTHONPATH=$PWD/tests <py> -m pytest \
  tests/injection/test_shard_coverage.py -q -p no:cacheprovider --noconftest -rA
```
- `--noconftest` ⇒ **不触发 `conftest.py::pytest_sessionstart → _clear_work_dir()`** ⇒ **不占删除配额**；
- ★ **并且带了正对照**：`test_quota_attribution_never_passes` **绿**（它与失败那条**走完全相同的加载机制**）
  ⇒ 证明"**红的是被检对象本身，不是我的探针**"。
⇒ **升格为口径**：**在配额受限期做验证，一律优先"不建夹具 + 正对照"的形态**。

### 更正 · `ws/ch13r-rule-candidates` 的 4 处引用**不是"悬挂引用"**（`ws-step56-skipped` 更正，主理人采纳）
`main` 的 `batch13_taskbook.md` 有成文裁决（`R1`：「**废弃 · 禁止合并** ……若合并该分支，`registry/rule-candidates/` 会复活 ⇒ 违 `G-06`」）
⇒ 5 个文件缺失是**预期状态**；那 4 份报告的引用**正是"讲清为何不得复活"的引用**
（`ws_ch2_rules_consumption_review.md:800` 逐字写着"**不得复活**"）。
★ **主理人先前判"悬挂引用"是错的** —— 更正：**它们是正当引用**。

### 主理人认错 · `PYTEST_MISSING_MARKER` 缺口**是我造成的**
`ws-step56-skipped` 实测计数：`90e58b0^` = **0** · `90e58b0` = **3** · `main@28e1eff` = **0**
⇒ 该提交的"测试 + 报告 + CONVENTIONS"都进了 main，**同一提交对 `verify.py` 的那段实现没进**
⇒ **根因是我在 `ea0ceae` 的整文件 `--ours`**（见 13.11 后的作用域错误条目）。**已复原（`1f58b16`）。**
★ 该 marker 存在的**全部理由**就是"把 0.1s 全红归因成解释器问题、别读成测试坏了" —— **正是 `G-60`/`G-61` 期最缺的能力**；
而**守着它的断言在 main 上、能力本身不在**。⇒ 这类"断言进了、实现没进"是**新增的红灯形态**，已并入口径 11 的第 4 种失效模式。

### 裁定 · verify 分片的「静默红」怎么处置
`run_all_gates.py` 与 `pre-commit.sh` **都不跑 verify 分片** ⇒ `injection-f` 的红**只有显式跑才看得见**（静默红）。
- ❌ **不接 `pre-commit`**：会显著拉长提交耗时；且 `G-61` 已证明 pre-commit 是"判据与对象易不同源"的高危面。
- ✅ **接"零删除的轻量代理"进 `run_all_gates`**：只做 `--collect-only` 的**片大小（≤32）+ 穷尽性（目录文件全集 ⊆ 分片）+ 两两不交**
  ⇒ 这类红（片越限/漏片）**立刻在门禁可见**，而**不需要跑 205 例**（**零删除配额**）。
⇒ 立卡交 `ws-step56-skipped`（它已用该手法做出判据）。

### 13.13 **更正 13.12 的两处**（`ws-degrade-contract` 自纠 + 主理人更正自己引用的数字）

**(1) 主理人更正：`base = 99,998` 是它**早先的推断值，已被它自己撤回**。
它做了**只读反解**（被拒 ⇒ 未删 ⇒ 目标仍在 ⇒ 可数出真实条目数），用 `count − 条目数` 求残差：

| 目标规模 | 观测 `count` | 残差 |
|---|---|---|
| 小（1~4 项） | `99,999` / `100,001` | **99,997~99,998** |
| 大（251~332 项，夹具目录） | `100,113`~`100,270` | **99,806~99,938** |

两个混杂效应**无法从报文分离**：① base 是**缓慢累积量**（同一 307 项目标，21:49→22:17 的 `count` 从 `100113` 爬到 `100116`）；
② **守卫的体积口径 ≠ 人数的"文件+目录"计数**（22:45 大目标残差 99,938 vs 22:47 小目标 99,998，隔 2 分钟差 60）。
⇒ ★ **正确表述只有**：「**base 长期贴近天花板（≈99,8xx~99,99x）且单调不减**」；**不要**引用任何具体 base 数值。
★★ 这正是 **`G-62`「静默等价态」的又一例**：「`targetCount: 1`」被读成"体积 1" ⇒ **字段语义未验明**就代入模型。

**(2) ★★ 报文级直证（比任何模型都硬）：此刻连"删 1 个文件"都被拒**
```
22:23:05  count=99999  targetCount=1
          target = .../ws-daily-nochange/system/tests/.work/gp-1-10274/f0.txt
```
现场核验：该目标是**单个文件（1 项，1 byte）**，且**至今仍在**（被拒 = 未删）。
⇒ 把两条结论从"推断"升级为**直证**：
1. **13-F 的"逐文件删除"在本回合连第一项都删不掉**（不是"只救不了下一回合"，是**本回合也救不了**）；
2. **逐文件还改变失败形态**：`base < T` 但 `base + 体积 ≥ T` 时，批量删 = **一次原子拒绝、夹具原封不动**；
   逐文件 = **先删掉一部分再被拒** ⇒ 夹具停在 **"半清空"**，而 `pytest` 只看到一个 `E`
   ⇒ **"清了一半"与"清干净了"观测上不可区分** ⇒ `G-62` 家族。

### 13.14 ★★ 裁定：把夹具工作目录移到**系统临时目录** —— **属「合法的工程取舍」**，但带**四条约束**

**判据（区分"取舍"与"绕过"的唯一问题）**：**这个动作是否削弱了该守卫本来要保护的对象？**
- 守卫辖域 = **仓库内**，其**保护对象**是仓库文件 / 用户工作成果；
- 夹具是**一次性 scratch**（`tests/.work/`，已 gitignore、每轮本就会被清空）⇒ **把它放 `/tmp` 并不触碰守卫要保护的东西**；
- 该守卫**从未以"保护 `tests/.work/`"为目的**（它是**附带**命中）。
⇒ **这是让工具与其目的重新对齐，不是规避** ⇒ **可用其绿作证据**。
★ **与"逐文件删除"性质不同**（`ws-degrade-contract` 的区分正确）：逐文件**仍在边界内**，只是粒度变小；本项是**位置**变化。

**四条约束（缺一即退化为绕过）**：
1. ★ **必须验证可行性**（主理人项目记忆里有一条反证："工作区外 `mkdir` 走 broker → `PermissionError`"）⇒ **最小实验先做**（已派 `ws-step56-skipped`）；
2. ★★ **不得设为默认** —— 必须**显式开关**（环境变量 / `--work-dir`），**默认仍是仓库内**。
   理由：**固化为默认会永久性地抹掉"仓库内删除配额"这个信号** ⇒ 等于**把一条真实运维约束从视野里删除**；
3. ★★ **每次启用必须打 note + 计数**（`G-03`：我们**用过**这条边界，这件事必须**可见**）；
4. ★★ **不得用它替代 `G-60` 的修复** —— **卡 13-M 仍须做**（把"每例副本条目数"降下来）；
   且本方法**只用于一次性测试夹具**，**不得**用于仓库内的批量删除/清理。
★ **代价须显式登记**：失去"仓库内可追溯 / 被 `.gitignore` 管理"；**`/tmp` 有独立系统清理 + macOS 定期清 `/tmp`** ⇒
**夹具可能被动消失 ⇒ 偶发假红**（`G-62` 家族）⇒ 实现时必须**检测"工作目录是否仍在"**并响亮失败。

### 裁定 · 两项请示
1. **§2.6 的三处不精确（含等号 / "体积"读不出 / base 数值）⇒ 单开一次订正提交：批准**（值得 —— 它是下游反复引用的判据）。
   要求：**合并成一次**提交；按**口径 11** 合完**再核 `main..b == 0`**。
2. **起草 13-F/13-M 验收判据三条的可测写法（含"半清空"的判定）：批准**，**不得动 `conftest.py`**。
