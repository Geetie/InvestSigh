...
| **`G-RC-10`** | ★ **夹具副本偶发不完整**（**非确定性假红**）。实测（同一轮内跑 `tests/guards` 三次，三次结果互不相同）：<br>· run1 → 只有我改的那条元断言红（63 passed）；<br>· run2 → `test_verification_policy.py::test_missing_broker_env_in_run_pytest_is_rejected` **ERROR**；<br>· run3（单跑该文件）→ `test_verification_policy.py::test_batch_target_outside_tests_is_rejected` **FAILED**，报<br>　`FileNotFoundError: …/tests/.work/test_batch_target_outside_tests_is_rejected-a9e0992e/system/scripts/ops/verify.py`。<br>⇒ 三次**都**报"**副本里应有的文件不存在**"，但**落在不同用例上**。<br>★ 关键排除：`conftest.code_root` 的 `copytree` 失败会**抛出**（`try/finally` 只做 rmtree，不吞异常）⇒ 这不是"copytree 静默失败"，而是**副本在半途缺件**（与宿主 FS broker / 删除配额压力相关：本轮我短时间内跑了 3 次 `tests/guards`，且已知单轮删除配额 9999，见 `G-RC-09`；早前一次探针的 brokered `copytree` 就**成批报过** `[Errno 2] No such file or directory`）。<br>★ **危害**：症状是"某条守卫测试随机红"，**极易被误判成"守卫坏了"**——而本项目已被这种假缺陷坑过多次（`G-RC-03` 的假红、`G7` 的死代码）。 | **高**（非确定性） | `OPEN` —— **必须在新一轮（干净配额）里复现与定位**：① 单轮只跑一次 `tests/guards`，记结果；② 连续多轮各跑一次，比对失败是否消失 ⇒ 若消失，即确认"配额压力导致副本缺件"；③ 若仍在，则与 `copytree`/`_make_writable`/`_reset_truth_source` 的**并发或顺序**有关，需逐步复现。**在定位前不得把 `tests/guards` 的红当作真缺陷** |
