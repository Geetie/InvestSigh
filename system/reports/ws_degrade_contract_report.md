...
| **③ ★ 本次未复核时效 → 不得读作"成立"** | `L513` / `L514` / `L515`（`result.blocked` 恒真 · `status` 恒 `failed` · **永远没有一行 `status=done`**）· `L518`（`last_valid_result_ref` 恒 `None`）· `L525` / `L526` / `L532` | **量不到 ≠ 不成立**（`V-11` 规矩 4 对本报告**自我适用**）：这些断言依赖 `T-18` 是否已被裁定落地，本次未验 ⇒ 只报"未复核"。★ **`L513`–`L518` 这一条已在本节下方实测掉**（改判为"当前成立、但样本仅 2 行"，并附 `G-63` 的可达性含义）；`L525`/`L526`/`L532` 属建议与铁律引用，随 `T-18` 定 |
...
**★★ ③类里那一条，本轮已经实测掉了（不留在"量不到"）** —— 对象 = **`main:system/facts/tasks.jsonl`**，
只读（`git show`，**不跑 pytest ⇒ 零夹具、零删除预算**），仪器 `python 3.13.12` + `json`：

```
[对象] main:system/facts/tasks.jsonl  →  行数 = 4
[地板真值] 其中带 check_record 的行 = 2          （>0 ⇒ 读数非空，仪器在该范围有产出）
[读数·状态] {'failed': 2}
[断言1] status == "done" 的 check 行 = 0                     → 成立
[断言2] last_valid_result_ref 非 None 的 check 行 = 0         → 成立
[断言3] status == "done" 且 output_refs 非空（= 载体③ 活体）= 0
[附]     全部 4 行里 output_refs 非空的 = 0
```

⇒ ③类那三条**当前不假**，与 §2.1 `:168` 的读法一致。**但半径要写清（`V-11`）**：
- **成立，但只是"在这个对象上成立"**：真源**整份**就是 4 行，我读的是**全量**（不是抽样）⇒ 举证半径 = 结论半径；
- **样本本身极薄**：2 条 check 行、**0 次成功运行** ⇒ 这些断言是**在"没有正面样本"的仓里成立**，
  它们证明**当前不假**，**不证明**将来仍成立 —— `T-18` 落地那天，"恒"字断言成片翻面。
- **★ 对 `G-63` 的直接含义**（供该卡开工时引用）：`G-63` 的**两个**落点都要求"前序行带真/假引用"这一前提，
  而当前生产真源里 **2 条 check 行的 `last_valid_result_ref` 全为 `None`**
  ⇒ **两个方向在当前生产数据上都不自然可达**。这正好印证审计给出的强度写法（`G-03`）：
  该卡的强度是"**判据边界可被机器证明不严**"，**不是**"已发生的违规"。
