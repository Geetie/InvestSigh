...
> ★★★ **本段整段作废（同轮再更正，见 6.5-⑫）**：下面说的"守卫开的数据我拿到了"**是假的** ——
> 6.6 **也是配置 `E`**（`verify.py::_child_env()` 从 `3653c7c` 起就**硬写** `CODEBUDDY_SAFE_DELETE_ENABLED="0"`，
> 外层设什么都无效）。**保留原文不删**作为记录，但**不要引用本段的任何一句**。
> ⇒ 正确的说法是：**(甲) 已于 `5b9bdc3`（23:09:19）被裁定**，`3653c7c`（23:21:44）把它落进
> `verify.py`/`run_pytest.sh` ⇒ **`E` 就是门禁口径**，故 6.3/6.6 **两张表是同一配置**。
- ~~同轮稍后我在**完全不加任何 env**（= 生产配置）下把 **23 个批次全跑了一遍**，
  **每一个都跑通、守卫一次都没触发**（6.6 全表 `exit=0`，无一条 `SAFE_DELETE_BULK_CONFIRM_REQUIRED`）。~~（同上作废）
- ~~⇒ 上面理由 ①（"本会话配额已耗尽且不恢复 ⇒ 守卫开时一个夹具批次都跑不出"）**在事实层面站不住**：
  **同一会话、稍后的时刻，守卫根本没有介入。**~~（同上作废 —— 6.6 里守卫"没有介入"是因为**它被强制关了**，
  不是因为"配额恢复了"，所以这句话**不构成**对理由 ① 的反驳）
- ⇒ **理由 ① 是否成立，我仍然不知道** —— 我**没有**守卫开的数据，**不声称**它已被推翻（6.5-⑫ 的结论面）。
- ⇒ **我 6.5-② 那条"按会话累积、不随轮次恢复"的结论仍然只是"至少不精确"**：判它的**证据不成立**，
  但**反向证据也没有** ⇒ 状态**回到"未解"**，与下面这两条观测并列（我保留原文不删，作为记录）。
  ...
  - 稍后时刻：23 个批次（含 `guards`/`pricelayer`/`injection-b..d` 这些夹具重的）**全绿、零告警**
    —— ★ **但这条观测的解释力已被 6.5-⑫ 抽掉**：那时守卫是关的，所以"全绿"**本来就不需要配额恢复**。
  ⇒ **登记为未解**，并把**可判定信号**留给下一个人（6.5-⑪ 末段）：先看有没有那行 `BULK_CONFIRM_REQUIRED`。
- ⇒ **对裁定的影响**：**(甲)(乙)(丙) 之问已被 `5b9bdc3` 裁定为 (甲)**（这是 6.5-⑫ 更正后的口径）
  ⇒ 6.3 的"按 `口径 21` 不得用于定值"**对 6.6 不适用**，两张表**同配置、同等合规**；
  **超时建议以 6.6 为准**这一点**不变**，但依据是"`E` = 已裁定的门禁口径"，**不是**"守卫开也这个数"。
...
### 6.6 ★★★ 门禁口径（= 配置 `E`）全表 —— 原说"必须新会话才能补"的 20 格，本轮补完了

> ★★★ **标题已改口（同轮，见 6.5-⑫）**：原标题写的是「生产（守卫**开**）」—— **那是错的**。
> 本节的 23 个数**全部在配置 `E`（守卫关）下取得**，因为 `verify.py::_child_env()` 自 `3653c7c` 起
> **硬写** `CODEBUDDY_SAFE_DELETE_ENABLED="0"`（实测探针 `child env = '0'` ⇒ `guard on in child? False`）。
> **读数一行都不用改**（配置本来就是 `E`），要改的只是**为什么它算"生产"**：
> `E` 已由主理人于 `5b9bdc3`（23:09:19）裁定为 **(甲) 合法配置**，并由 `3653c7c`（23:21:44）
> 落进 `verify.py`/`run_pytest.sh` 的注释「`E` 下即生产/门禁口径」⇒ **`E` 就是门禁自己的口径**。
> ⇒ 故 6.3 与 6.6 **是同一配置的两次独立测量**（差异 = 宿主负载，**不得归因给守卫**）。

★ 6.3 那张表是**我手工设 `E`** 的表；6.6 这张是**门禁自己强制 `E`** 的表 —— **配置相同**。方法：
`verify.py --batch <名>` **一路不加任何 env**（★ 但外层 env **不参与**：`_child_env()` 三项全部显式覆盖）
⇒ 走的是**门禁自身的配置**。**每批仍是独立的 `verify.py` 调用、互不并发**
...
★ **第二样本（同轮，`#91` 提交后复跑；`n=2`）**：上表是**单次**读数，我另取了三个受 `#91` 改动影响的批次的第二次：

| 批次 | 第 1 次 | 第 2 次 | 两次/慢者 |
|---|---|---|---|
| `injection-f` | 7.32s | **3.84s** | 1.9× |
| `injection-g` | 5.67s | **6.05s** | 1.07× |
| `guards` | 13.23s | **16.02s** | 1.21× |

⇒ **单样本不要当常数用**（`injection-f` 两次差 1.9×）。这三个批次即使取慢者也仍在 20× 量级，
⇒ **本表的"无一批 < 4×"结论不变**；★ **但另 20 个批次只有单样本** ——
**不要**据本表断言它们同样稳定（6.3 已实测"宿主并发可把同一片拖慢 11.8×"）。
⇒ 定值时按**两次的慢者**算（`V-02` 的"取上沿"口径）；只有单样本的批次，取该样本 × 8 并**接受它是下界**。

**★★ 守卫开 vs 守卫关 —— ★★★ 整张表作废（见 6.5-⑫）**

> **原文（作废，保留作记录）**：~~「守卫开 vs 守卫关（同一台机、同一棵树、相邻时刻）—— 这是本卡最该被记住的一组数」~~
> ~~| 批次 | 守卫**关**（6.3，`E`） | 守卫**开**（6.6，生产） | 开/关 |~~
> ~~| `unit` | 2.59 / 1.99 | 3.40 | ≈1.3× | `guards` | 5.85 / 5.52 | 13.23 | ≈2.3× |~~
> ~~| `pricelayer` | 11.74 / 10.68 | 15.36 | ≈1.3× | `conflict` | 0.42 / 0.38 | 0.50 | ≈1.2× |~~
>
> **为什么作废**：两列**是同一个配置**（6.3 我手工设 `E`，6.6 被 `_child_env` 强制 `E`）。
> ⇒ 我**让一个根本没变的变量"变了"**，并把它算成"守卫的价钱" —— 这比 `口径 12`（两个变量同时变）
> 更糟，是**零个变量变化下的归因**。差值（1.2~2.3×）只能记作**宿主负载噪声**（6.3 已实测过
> 同一片可有 **11.8×** 的并发波动）。
> ⇒ ★ **`guards` 5.85→13.23 这个 2.3× 尤其要撤回**：它看起来"最像守卫成本"（夹具最重），
> 但恰恰因此**最像并发噪声**。**不要**拿这张表去说"守卫只贵一点点"。

★ **仍然成立的两条**（不依赖那张表）：
- 主理人最初实测 **48.73s / 66.72s**（那时 `raw/` 还在被复制进来再逐个删掉）——
  **真正把成本拿掉的是那个修复（`_COPY_SKIP` + `raw`），不是超时值**；
- 门禁口径下 `unit` 载荷是 **3.40s**（6.6），`unit` 的 270s **确实只是安全网**。
- ⇒ ★ **但"修好之后守卫开着只贵 1.3~2.3×"这句我撤回** —— **我压根没有守卫开的数据**（6.5-⑫）。

**★ `unit` 超时的新建议（本卡第 4 条要求"若真降了就重给，但不许压回 60s"）**：
**维持 270~300s，不要往下压。** 理由**不是**保守，而是一个**双机制模型** ——
★ 但两个机制的**可达路径不同**（这是 6.5-⑫ 更正后的精确版）：

- **机制 A（删除量在阈值下）**：门禁口径实测 `3.40s` ⇒ 按 ×8 只需 **30s**；
- **机制 B（删除量撞阈值，守卫转入审批回合）**：主理人实测 **48.73s / 66.72s**
  ＝我的 `2.59s` 的 **19~26×**；这不是假设，是**同一批次的实测**。
  ★ **机制 B 在 `verify.py` / `run_pytest.sh` 下不可达**（守卫被强制关，6.5-⑫）。
  它可达的路径是**绕过包装器直接跑 pytest**：本仓 Bash 工具环境实测
  `CODEBUDDY_SAFE_DELETE_ENABLED='1'`（守卫**默认开**）⇒ 任何 `python -m pytest tests/...`
  或 IDE 里点运行，都走机制 B。

⇒ 超时值必须按 **max(机制 A, 机制 B) × 4~8** 定 ⇒ `66.72 × 4 = 267` ⇒ **270s**（当前值）**站得住**；
`300s`（本轮 `main` 已改到的值）也在同一区间。**任何按机制 A 定的值（≤30s）都会在机制 B 下随机红**
—— 而"机制 B 会被走到"这件事**不取决于门禁**（它取决于人会不会绕过包装器），
故 ★ **我仍建议维持 270~300s，但这条建议的性质是"防误用"，不是"防门禁自身"**（口径变了，如实声明）。
...
⑧ **一个仍需下一轮的空白**：配置 `E` 下的表已全；**守卫开（=生产）**的表只有
   ...
   ★★ **同轮更正（原文，保留）**：~~这条空白本轮已补完 —— 见 6.6（23/23 行，生产配置，零 env 覆盖）~~。
   ★★★ **再更正（6.5-⑫）：上句是错的** —— 6.6 **也在 `E` 下**（`_child_env()` 强制），
   ⇒ **本项空白一个字都没被填上：我至今没有守卫开的 23 行表，一行都没有。**
   原文 ①（"'必须在新会话'这个前置没有被证实"）**撤回**（那次"同会话能跑"是因为守卫被关了）；
   原文 ②（"不要把 6.3 当成守卫开的数"）**正确，且现在是唯一还站得住的一句** ——
   而按 6.5-⑫ 的更正，**6.6 也不能当成守卫开的数**（两张表都是 `E`）。
   ⇒ ★ 剩下的唯一真问题：**门禁口径 = `E` 已被裁定**，所以"守卫开的表"**不再需要补**
   （它只是"绕过包装器时会怎样"的诊断，不再服务于定值）。
...
⑪ ★★ **"红的理由被吞掉"这条我复现不出 —— 它是`条件性`的，不是该断言的固有属性**（按 `V-11` 收窄主语）：
   ...
   - ★★ **本项（⑪）的第 491~494 行同时被 ⑫ 抽掉了解释力**：那里把"守卫一次都没触发"读成
     "守卫是否触发是会话/阈值状态依赖的"——**那个推论不成立**，因为当时守卫**被强制关着**。见 ⑫。

⑫ ★★★ **6.6 不是"守卫开" —— 它是配置 `E`；连带作废三处结论（本轮最重的一条自纠）**

   **怎么发现的**：在 `#102` 收尾时读 `verify.py::_child_env()`，看到它**三项都显式覆盖**：
   ```python
   return {
       **os.environ,
       "CODEBUDDY_SAFE_DELETE_SANDBOX": "0",
       "CODEBUDDY_BROKERED_FS_HOOK_ENABLED": "0",
       "CODEBUDDY_SAFE_DELETE_ENABLED": "0",     # ← 第三道闸门：外层设什么都无效
   }
   ```
   ...
   **一、决定性证据（代码 + 实测，两条独立）**
   ...
   | # | 证据 | 原文 |
   |---|---|---|
   | 1 | **代码**：`git show <c>:system/scripts/ops/verify.py \| grep -c CODEBUDDY_SAFE_DELETE_ENABLED` | `785464c`(23:20:11)=**0**、`6af4ed8`(23:19:35)=**0**、`8c38bfb`=**0**；**`7993d01`(23:34:41，写下 6.6 的那个提交)=2**、`d3f5701`=2、`b7c0c8f`=2 ⇒ 6.6 的测量**发生在第三道闸门落地之后** |
   | 2 | **实测探针**（import `verify.py` 后直接调 `_child_env()`） | `caller shell env CODEBUDDY_SAFE_DELETE_ENABLED = '1'` / `child env CODEBUDDY_SAFE_DELETE_ENABLED = '0'` / `=> guard on in child? False` |

   ★ 第 2 条**同时钉死了另一件事**（超出本卡、但值得全队知道）：**本仓 Bash 工具的环境里
   `CODEBUDDY_SAFE_DELETE_ENABLED=1`（守卫默认开）**，而 `_child_env()` 把它覆盖成 `0`
   ⇒ 6.1 那条守卫路径**只在绕过包装器时**才走。另：`run_pytest.sh:35` 也硬写同一个值（`3653c7c` 加的）。

   **二、被作废/改口的东西（三处，全部主动撤，不等裁）**

   | 我写过的 | 现在的状态 |
   |---|---|
   | 6.6 标题「生产（守卫**开**）」+ 方法段「`run_pytest.sh` 只关 broker，不关守卫 ⇒ 走 6.1 完整守卫路径」 | ★ **错，已改口**。6.6 **是 `E`**；改后标题「门禁口径（= 配置 `E`）」，并说明它为何**仍算生产**（(甲) 已裁定 + `3653c7c` 落地） |
   | 6.6 末尾「**守卫开 vs 守卫关 ≈1.3~2.3×**」整张表 | ★ **整表作废**。两列**同配置**（6.3 手工 `E` / 6.6 强制 `E`）⇒ 我**让一个没变的变量"变了"**，比 `口径 12` 更糟（零变量归因）。差值只能记作**宿主负载噪声**。**`guards` 5.85→13.23 的 2.3× 尤其撤回**（它最像"守卫成本"、也最像并发噪声） |
   | 6.6 里 `unit` 270~300s 的**机制 B** 论证 | ★ **前提改口**：机制 B 在 `verify.py`/`run_pytest.sh` 下**不可达**（守卫被强制关）。它可达的路径是**绕过包装器直接 `pytest`**（本仓 Bash env 实测 `=1`）。⇒ **结论不变（维持 270~300s）**，但**性质变了**：从"防门禁自身的机制 B"改成"**防误用**" |

   **三、被牵连、必须一起说的两处**

   - **6.3 的"重大更正"段**（"因为守卫开的数据我拿到了"）：**整段作废**。特别是它拿 6.6 当
     证据去否定"本会话配额已耗尽且不恢复"（理由 ①）——**那个反驳不成立**：6.6 里守卫"没介入"
     是因为**它被关掉了**，不是因为"配额恢复了"。⇒ **我的 6.5-② 那条"按会话累积、不恢复"
     回到"未解"**（判它的证据不成立，反向证据也没有），而不是"被推翻"。
   - **6.5-⑪ 的第 491~494 行**（"守卫是否触发是会话/阈值状态依赖的"）：**该推论撤回**，
     同样因为当时守卫是关的。

   **四、没被推翻的部分（6.6 的数据本身仍然有效，一个字都不用改）**

   - 23 行的 `<elapsed>/<timeout>`、`exit` 码、**"倍数 < 4× 的批次 = 0"**、`unit` 3.40s、
     `pricelayer` 15.36s … **读数全部保留**（配置本来就是 `E`，所以数没变）；
   - 6.3 那段"按 `口径 21` 不得用于定值"**对 6.6 不适用**：`E` 已由主理人在 `5b9bdc3`（23:09:19）
     裁定为 **(甲) 合法配置**，且 `3653c7c`（23:21:44）已经把它落进**门禁自己的代码**（注释原文：
     「`E` 下即**生产/门禁口径**（CI 亦然）」）⇒ **`E` 就是门禁口径**；
   - ⇒ **结论收敛成一句**：6.3 与 6.6 **是同一配置的两次独立测量**；**6.6 仍合规定值**，
     但**理由**从"守卫开着也是这个数"换成"**`E` 是已裁定的门禁口径**"。

   **五、教训（可用的一条）**：**"我没设 env" ≠ "子进程的 env 是继承的"。**
   凡结论建立在"我用了生产配置"上，必须**读出子进程实际拿到的 env**（本例一行探针即可），
   而不是读出"我没动过它"。★ 这与 `V-11`「仪器轴」同族：**仪器 = 被调用的包装器 + 它自己设的 env**，
   不是"我敲的那行命令"。

⑬ **`pricelayer` 的例数在我订正之后**又涨了（`#91` 的"删写死数字"政策被现场再次验证）

   `#91` 里我把 `pricelayer` 的"现算 183"写进了 `verify.py` 与 `CONVENTIONS.md`（漂移记录）。
   **`#102` 收尾复跑同批**：`192 passed in 12.98s`（`13.28s/300s`）；**随即 `git merge main` 后再跑：`201 passed in 14.61s`**
   （`14.84s/300s`）⇒ **183 与 192 都过期了**。
   轨迹：**127 → 168 → 183（`#91`）→ 192（`#102`，合并前）→ 201（`#102`，合并后）**。
   - ★ **我在同一个收尾轮里亲眼看着它变了两次**（192 是我在合并前测的，提交前就变成 201）
     ⇒ **"写死例数"这个做法本身在任何时间点都会过期**，唯一稳的写法是**指真源**。
   - 我已在 `verify.py` 里把那两处**改成快照 + 政策表述**（"当时的 183 例"+"一直在涨，别当基准"），
     **不再写"现算"** —— 写"现算"的数字**下一次就会变成假证据**。
   - `192` 这个数我**特意没有从报告里删掉**：它是"写死数字活不过一轮"的**现场证物**。
   - ★ **`CONVENTIONS.md:96` 还有一处**（`pricelayer 写 168 实为 **183**`，`#91` 的漂移记录块内）
...
- **V-02 表**：超时列 6 行订正；**删掉所有写死的例数**（含 `pricelayer 168 例`/`valuelayer 206 例`）；
  **补 `injection-g` 整行**；`injection-f` 的行内描述同步去掉「分片绑定」（`#89` 已把它移走）；
  ...
  ★ **6.5-⑫ 更正：`CONVENTIONS.md` 里最终写下的表头是「生产配置」**（我落笔时没写"守卫开"），
  **故 `CONVENTIONS.md` 无需回改**；错的是本报告里这句描述（6.6 是配置 `E`，而 `E` = 门禁口径 = 生产）。
...
- ★ **另加一条自我更正（不是 `#91` 的条目）**：同一附录的「**配额触顶后，本轮内不恢复**」
  **已被我本轮实测收窄**（6.6：同一会话、零 env 覆盖、23 批全通、零告警）⇒ 改成
  ...
  ★★ **6.5-⑫ 更正**：上句的**理由**（"6.6 证明了会恢复"）**不成立**（6.6 里守卫是被关的）。
  ⇒ **落笔结果是安全的、不必回改**，因为收窄后的写法**只保留事实、不写规律**；
  正确的理由应是：**"不恢复"这条推广本来就没有建立过**（不是"被反驳"）。
...
| 原条目 | 终态 | 证据 |
|---|---|---|
| 1 `evidence`/`daily`/`injection-a..f` 未取得实测 | **已关闭** | 稍后**同一会话**把 23 批全跑通（**6.6**，门禁口径 = `E`）。⇒ "必须新开轮次"这个前置**未被证实**（机制仍未解，见 6.3"重大更正"）★ **6.5-⑫ 更正**：那次"同会话能跑通"**不是**因为"前置未证实"，而是因为**守卫被强制关了**（`_child_env`）⇒ 本行的"已关闭"**只对 `E` 口径成立** |
| 2 `unit` 120s 在满负载下可能不成立 | **已关闭（但建议不变）** | `main` 已把 `unit` 抬到 **300s**；门禁口径实测 **3.40s ⇒ 88×**。**新建议：维持 270~300s**，理由是**双机制模型**（见 6.6 末段）★ **6.5-⑫ 更正**：机制 B 在门禁下**不可达**，故该建议的性质是"**防绕过包装器**"，不是"防门禁自身" |
...
> ★★ **同轮结论（★ 已被 6.5-⑫ 更正，见下）**：~~本清单的主项（守卫开的生产全表）已在同一会话里做完了~~。
> ★★★ **6.5-⑫ 更正**：不成立。6.6 **也是 `E`** ⇒ **守卫开的生产全表至今未做，一行都没有**。
> 下面这段前置的**适用范围**也随之变了：它**不是**"能不能取到生产数"的门（生产数 = `E` 数，已全），
> 而是"**要不要**再取一次守卫开数"的门 —— 而按 (甲) 裁定，**那个数不再服务于定值**，故**本清单可整体搁置**。
> ~~实测反例：本轮稍后没换会话、没加任何 env，23 个批次全部跑通（`exit=0`，零 `BULK_CONFIRM_REQUIRED`）。~~
> ⇒ ★ **纪律修订建议**（**这条我要撤回并改口**）：~~先在当前会话试一次并看有没有那行 `BULK_CONFIRM_REQUIRED`，
> 有 ⇒ 才谈换会话；不要因为"上次撞过"就预设这次也撞（这次我就白等了一轮）。~~
> ⇒ **改后**：**先确认这次的子进程守卫到底开不开**（读 `_child_env()` / 一行探针），**再**决定要不要按
> "配额已耗尽"处理。我原来那条建议**跳过了这一步**，于是把"守卫被强制关"读成了"配额恢复了" ⇒
> **结论对不上现场，且我当时还把它当成一条纪律往外发**（`V-10`/`V-11` 的"仪器轴"正是这个错）。
...
#   ★★ 同轮更正（★ 已被 6.5-⑫ 二次更正）：~~这不是一条长期判据 —— 稍后同一会话里 23 个批次全跑通、
#      守卫一次未触发（见 6.6）。~~ 那次"守卫一次未触发"**不能**用来反驳本判据 —— 当时守卫**是被关掉的**。
#      故本判据**仍然只是一条"此刻"判据**（方向不变），但**理由换成**："守卫是否生效取决于子进程 env，
#      不由会话状态决定" ⇒ 用它之前必须先确认守卫真的开着（`_child_env()` 探针）。
```

★ **临时开关 `CODEBUDDY_SAFE_DELETE_ENABLED=0` 的用法与边界**（见 6.5-⑥，**未提交、需主理人裁**）：
它让守卫整体不生效 ⇒ 测量可跑、口径与**当时我以为的**生产**不同**。
★★ **6.5-⑫ 更正**：这个"不同"**已经消失** —— `E` 被裁定为 (甲)（`5b9bdc3`）、并已由 `3653c7c`
写进 `verify.py::_child_env()` 与 `run_pytest.sh:35`，注释原文即「`E` 下即生产/门禁口径（CI 亦然）」。
⇒ **`E` 不再是"临时开关"，它就是门禁口径**；**6.3 的全表与 6.6 的全表因此是同一配置下的两次测量**。
★ 原文此处写"守卫开（生产）的数只能在新会话里取" —— **该说法本轮已被自己推翻**，但
~~6.6 就是在同一会话、零 env 覆盖下取的 23/23 行生产表~~ ★★ **推翻的理由是错的（6.5-⑫）**：
6.6 能在同会话跑通**不是**因为"前置未证实"，而是因为**守卫被强制关了**。⇒ 该说法**状态回到"未解"**。
...
- ★ **本轮为取 6.6 全表，跑了 23 次 `verify.py --batch`（门禁口径 = `E`）+ 2 次单文件 pytest**，
  **全部零 env 覆盖**；分若干组在**同一 shell 内顺序**执行（组内无并发）。
  ⇒ ~~这**不与** 6.3 的越线（改 env）同类：6.6 的表**合规**（`口径 21`/`口径 22`）。~~
  ★★ **6.5-⑫ 更正**：这两句要分开看 —— ①**"合规"仍然成立**（`E` 已裁为 (甲)、且已是门禁自身口径，
  `口径 21`/`口径 22` 全满足）；②但**"不与 6.3 同类"不成立**：**6.6 也是 `E`**（守卫被 `_child_env` 强制关）。
  ⇒ 正确表述：**6.6 与 6.3 是同一配置的两次测量，二者同等合规**，差的只是"谁设的 env"（我 / 包装器）。
- ★ 本轮**没有**再用 `CODEBUDDY_SAFE_DELETE_ENABLED=0`（一次都没有）
  —— ★★ **6.5-⑫：这句话字面为真但会误导**：子进程**照样**拿到 `=0`，因为 `_child_env()` 显式覆盖。
  ⇒ **"我没设"不等于"它没生效"**。

### ⑦-补4：卡 `#91` 轮（同会话）

```
d3f5701  fix(docs): #91 片数/例数表述漂移逐处订正（verify.py 9+5 处散文、CONVENTIONS
         V-02 表+附录+V-08 一行）—— 删写死数字改指真源，并订正 V-02 表 6 行过期超时+补 injection-g 行
```

- 按 `V-07`：进 `#91` 前 `git stash push -u -- <两文件>` → `git merge --no-edit main` → `git stash pop`
  （**两文件自动合并成功、无冲突**）→ 立刻 `bootstrap_worktree.sh` ⇒ `BOOTSTRAP_EXIT=0`，
  `rules_lock_guard … RESULT: PASS（0 violations）`。★ 该区间 `main` 只改了 `CONVENTIONS.md` 的 **V-10/V-11 段**
  （8 个提交，均为他人工作），**未碰** `V-02`/附录/**`verify.py`** ⇒ 合并无交叉。
- **改动面 = 3 个文件**：`system/scripts/ops/verify.py`、`system/CONVENTIONS.md`、本报告。
  ★ **`verify.py` 只改注释与文档串**，机器核验见上（`BATCHES`/`INJECTION_SHARDS`/`ORDER` 的
  `name/label/argv/timeout/note` **逐项相同**，唯一不等项是 `verdict` 的函数对象）。
  ⇒ 因此**没有重跑全表**（无行为变更）；但按"相关批次必须复跑"复跑了 **`injection-f`/`injection-g`/`guards`**
  ⇒ 全 `exit=0`（读数见 6.6 的第二样本表）。`pre-commit` 逐次 `PASS（0 violations）`，**未用 `--no-verify`**。
- `git status --short` **空**；只 `git add` 上述三个显式路径（**无 `git add -A`**）。
- ★ **一条自我声明（可能被否决）**：`CONVENTIONS.md` 的 **`V-08` 段有一行**（"现在切成 6 片（`injection-a~f`）"）
  **不属于我 Phase-A 的交付面**，但它**是 `V-02` 真值的直接复述**，改与不改都会留下一个问题
  （不改 ⇒ 同文件内相隔三节自相矛盾；改 ⇒ 越界）。我**选择改并在 §`#91` 处声明**，请主理人裁。
