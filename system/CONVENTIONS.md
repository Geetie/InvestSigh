...
| 语法 | 裸BRE | `/usr/bin/grep` BRE | 裸 `-E` | `/usr/bin/grep -E` | 处置 |
|---|---|---|---|---|---|
| `\|` 交替 | **0** rc1 ✗ | 8 ✓ | 8 ✓ | 8 ✓ | 改 `-E`，或多 `-e` |
| `\+` / `\?` | **0** rc1 ✗ | 5 / 7 ✓ | 5 / 7 ✓ | 5 / 7 ✓ | 改 `-E` |
| `\<…\>` / `\b` 词边界 | **0** rc1 ✗ | 1 ✓ | **0** rc1 ✗ | 1 ✓ | ★ 改 **`-w`**（`-E` 救不了） |
| `\(…\)\1` 组 + 反向引用 | 1 ✓ | 1 ✓ | **0** rc1 ✗ | 1 ✓ | ★ **留在 BRE**（`-E` 会静默丢） |
| `\.` / `\{n,m\}` | 1 ✓ | 1 ✓ | 1 ✓ | 1 ✓ | 原样即可 |
| 纯字面 / `^` `$` 锚点 | 5 ✓ | 5 ✓ | 5 ✓ | 5 ✓ | 原样即可 |
...
1. ★★ **没有「无方言」的工具** —— 三个引擎三种语义，**同一个 `\|` 给出三个不同答案**：
   ...
   | `Grep` 工具（ripgrep / Rust regex） | **字面竖线** | 0 | 20 | ✗ **静默零** |

   ⇒ ★ **"改用 `Grep` 工具"不是 `\|` 的解** —— 这条推荐语本人写过、**已按实测作废**。
   `Grep` 工具同样是**方言**而非方言中立：**支持** `\b`（实测 8 = 真值 ✓），
   但**不支持反向引用**（`(.)\1` 实测 **0**、地板真值 **607**，**且不报错**）。
   **选工具的原则是"你知道它的方言"，不是"选个更高级的"。**
2. ★★ **凡结论建立在某次 grep 类输出之上，必须同时给出**：
   **引擎指纹**（`command -v grep` 绝对路径 / `--version` 原文 / 工具名 **+ 是否 ripgrep**）
   \+ **`rc` 原文**。★ `Grep` 工具**不暴露 `rc`** ⇒ 用它时必须**额外**给**地板真值**（本段第 3 条），
   否则连"零"和"出错"都分不开。**"零命中"结论缺这些即为无效证据**；
