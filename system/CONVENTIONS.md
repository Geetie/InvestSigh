...
4. ★ 管道里取退出码必须显式（`{ cmd; echo "EXIT=$?"; } 2>&1 | tail -N`；zsh `PIPESTATUS` 是 1-based，
   `${PIPESTATUS[0]:-$?}` 会**静默回落**）；
...
| **仪器轴**（本会话第 3 例，`ws-degrade-contract` 提供） | **样本来自另一个仪器** ⇒ 数字的"主语"不是它自称的那个 | ① **`口径 21`**：拿门禁口径 `49.35s` 比裸 pytest `66.72s` ⇒ 假"高方差"；② 复核 `_collect_count` 时用裸 `python3`（无 pytest）⇒ **七片全 `rc=1`、输出只有 `No module named pytest`**，得到"分片全废"的假象；③ **静态 `grep -c '^\s*def test_'` 数得 `f=30`，现算 `f=33`**（参数化展开，同一文件两种口径可差 **1~6 例**）⇒ **"没超限"不能靠数 `def` 得出** |
