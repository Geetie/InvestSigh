...
> 本方案确需改动前序文件方可堵住"参数新层被绕过"的缺口。**本文件不自行修改任何既有文件**，仅登记如下。格式：`目标文件 | 节 | 现状 | 改成什么 | 对应需求 | 关联冲突`。
...
> **说明**：这两条本质是同一件事——把**第十一章新引入的参数配置**补进 Ch2 §B.4 的**扫描范围**，否则"**不得把待定项补成新门槛**"（源稿第 386 行）在新层无守卫。**建议主理人在第十一章落地后统一登记到 `02/02 §F 回改清单`（新编号续接），并按"三段式审计链"回改 `02/02 §B.4`/§B.3。**
