...
| 项 | 内容 |
|---|---|
| **配置** | `rules/notification.yaml`：`notification_channels` / `notification_aggregation`，值 **`tbd`**（需需求方澄清，见 §K ③） |
...
> **本章不选一个当结论**（C8-2 需需求方裁定）。系统以配置 `rules/ask.yaml:gate_mode` **保持可切换**，默认与自动监测一致（走完整 gate）。
...
> 覆盖状态：✅ 已覆盖 ｜ ⚠️ 部分覆盖（依赖需求方裁定或实施参数冻结，非技术不可行）。
...
| N8.2-03 | P0 | 同 6 阶段 + 同 `evidence_gate`；`rules/ask.yaml:gate_mode` 可切换（§C.5） | 提问产出与自动运行同字段/同门槛 | ③ | ⚠️ **C8-2 待裁定（保持可切换）** |
...
| N8.3-03 | P1 | `rules/schedule.yaml`（窗口/时区/非交易日可配置，§D.1） | 调度含窗口声明；时刻/时区/非交易日为配置项 | ④ | ⚠️ 实施参数待实现方确认（第十一章） |
...
| N8.3-12 | P1 | `rules/publish.yaml`（值 `tbd`）+ `publish_hook`（Ch1 §H.1） | 默认无逐次批准；发布/复核政策为配置且值 `tbd` | ⑤ | ⚠️ **C8-1 待裁定（保持可切换）** |
...
| N8.4-10 | P1 | 承诺时效 ≤ `min(update_freq + latency)`（Ch9 §3.4.8）；文档无"实时/毫秒级"承诺 | 承诺时效不超来源频率支撑范围 | ④ | ⚠️ 时效目标待实现方评估（第十一章） |
...
- 优先级对齐 PM：**P0 31 / P1 8** ✅。
- **4 条 ⚠️ 全部非技术不可行**：N8.2-03 / N8.3-12（**需需求方裁定** C8-2 / C8-1，保持可切换）、N8.3-03 / N8.4-10（**实施参数待冻结**，第十一章）。其余 35 条可直接验收。
...
| # | 待明确 | 归属 | 处理（本章实现口径） |
|---|---|---|---|
| ① | **发布与人工复核政策**：默认自动更新 vs 人工批准 vs 对外授权发布的边界（C8-1） | **需需求方裁定** | 按"对象分层"口径实现，保持可切换（§D.1 / N8.3-12）：常规研究/图谱更新→默认自动；基准变更→人工审批（Ch9 §3.4.12）；对外公开→逐项授权；重大变化/争议推断→自动发布但标 `published_as=inferred` |
| ② | **提问是否走完整 gate + 资源排队规则**（C8-2） | **需需求方裁定** | 走同门槛，事实类可短路（`gate_mode`）；排队用 `priority_order`（§C.5） |
| ③ | **通知渠道与聚合方式** | 需需求方澄清 | 接口先定，值 **`tbd`**（`rules/notification.yaml`，N8.1-09） |
...
> 本方案确需改动前序文件方可堵住"新层守卫缺口"（S-01/S-02/S-15）。**本文件不自行修改任何既有文件**，仅登记如下。格式：`目标文件 | 节 | 现状 | 改成什么 | 对应需求 | 关联冲突`。
...
> **说明**：以上 3 条本质是**同一件事**——把"第八章新引入的**触发/通知/展示**配置"补进 Ch2 §B.4 的**扫描范围**，否则 P-09/P-10/N3.2-05 会在**新层（入口/通知/降级）**被绕过。**建议主理人在第八章落地后统一登记到 `02/02 §F 回改清单`（新编号续接），并按"三段式审计链"回改 `02/02 §B.4`。**
